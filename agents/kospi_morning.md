...
- 각 항목은 **평가(판단)를 문장 앞에** 두고, 수치는 뒤에서 보완한다. 수치를 나열하는 대신 "어떤 의미인지"를 먼저 쉽게 설명한다.
- 문장은 **토스 문체**로 쓴다: `~했어요`, `~해요`, `~있어요`, `~돼요` 등 해요체를 사용한다. `~합니다`, `~입니다` 사용 금지.
- 전문 용어는 최소화하고 누구나 이해할 수 있게 풀어서 쓴다.
- 수치는 `<b>`로 강조하되, 평가 뒤에 보조적으로만 사용한다.
...
**`scenario` 문체 규칙:**
- 종결 어미는 반드시 `~했음`, `~있음`, `~이끌었음`, `~충족`, `~예상` 등 **명사형**으로 끊는다. `~입니다`, `~합니다`, `~어요`, `~해요` 사용 금지.
- 어려운 금융 용어는 쉬운 말로 풀어 쓴다.
- MA20·MA200 실제 수치를 문장 안에 직접 언급한다 (예: "MA20(68,200원)을 넘어서며").
- 2~3문장으로 작성하되, 최근 흐름 → 지금 상태 → 앞으로 여지 순서로 전개한다.

예시:
```
전일 외국인이 대거 사들이며 반도체 업종 전반의 상승을 이끌었음. 단기 기준선 MA20(72,000원)과 장기 기준선 MA200(68,500원)을 모두 넘어선 상태로 추가 상승 여지가 남아 있음.
```

**예측 근거(`reasons`) 문체:** 최대 **4개**, 배열 순서가 중요도 순. 각 항목은 완전한 문장으로 작성. 이모지 1개를 앞에 붙이고, 수치는 `<b>...</b>`로 강조한다. **평가를 앞에, 수치는 보조로. 종결은 반드시 해요체(`~했어요`, `~있어요`, `~돼요`)로** 쓴다.
