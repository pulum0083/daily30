...
- 문장은 누구나 이해할 수 있는 쉬운 말로 쓴다. 전문 용어는 최소화하고 풀어서 표현한다.
...
- 어려운 금융 용어는 쉬운 말로 풀어 쓴다. 독자가 투자 초보자라도 이해할 수 있게 쓴다.
- MA20·MA200 실제 수치를 문장 안에 직접 언급한다 (예: "MA20(68,200원)을 넘어서며").
- 2~3문장으로 작성하되, 최근 흐름 → 지금 상태 → 앞으로 여지 순서로 전개한다.

예시:
```
전일 외국인이 대거 사들이며 반도체 업종 전반을 끌어올렸음. 삼성전자도 같은 흐름에 올라타 단기 기준선인 MA20(72,000원)과 장기 기준선 MA200(68,500원)을 모두 넘어선 상태임. 추가 상승 여지가 남아 있음.
```

**예측 근거(`reasons`) 문체:** 최대 **4개**, 배열 순서가 중요도 순. 각 항목은 완전한 문장으로 작성. 이모지 1개를 앞에 붙이고, 수치는 `<b>...</b>`로 강조한다. **평가를 앞에, 수치는 보조로** 쓴다.
