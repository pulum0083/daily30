...
**근거 문체 규칙:** 각 항목은 **평가(판단)를 먼저** 쓰고, 수치는 뒤에서 보조로 사용한다. 누구나 이해할 수 있는 쉬운 말로 쓰고, 전문 용어는 최소화하거나 풀어서 표현한다.
...
- MA20·MA200 실제 수치(달러)를 문장 안에 직접 언급한다 (예: "MA20($168.76)을 넘어서며").
- 2~3문장으로 작성하되, 최근 흐름 → 지금 상태 → 앞으로 여지 순서로 전개한다.

예시:
```
전일 빅테크 상승을 주도하며 반도체 섹터 전반의 모멘텀을 이끌었음. 단기 기준선 MA20($168.76)과 장기 기준선 MA200($161.76)을 모두 넘어선 상태로 추가 상승 여지가 남아 있음.
```
