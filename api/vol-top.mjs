// 거래량/등락 톱 실시간 API — 종목 41 네이버 장중 시세 병렬 fetch → 거래량·상승·하락 정렬용 반환
...
export default async function handler(req, res) {
  ...
  try {
    const stockRes = await Promise.all(Object.keys(UNIVERSE).map(c => fetchVol(c, UNIVERSE[c].name, UNIVERSE[c].sector)));
