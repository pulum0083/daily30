...
// ── 야간 추정가 (국내장 마감 후) ─────────────────────────────────────────────
// /api/hl-night = KRX 종가 × 하이퍼리퀴드 xyz dex 24h 무기한선물의 종가 이후 변동률(앵커 환산,
// 2026-07-31~). 비율만 쓰므로 HL 합성가의 상시 프리미엄은 상쇄되고 진짜 야간 변동만 남는다.
...
//  · 등락률은 응답의 changePct(서버가 앵커로 계산한 '종가 대비' 값)를 그대로 쓰지 않고, 화면에
//    실제로 보이는 종가(data-close 또는 히어로 값) 기준으로 다시 계산한다 — 둘이 다를 수 있어서다
//    (장중 곡선 IIFE가 히어로를 실측 1분봉 마지막 값으로 보정하는 경우). 라벨에 '종가 대비'를 명시한다.
//  · adjusted:true는 앵커 계산 근거(종가시점 HL가 또는 KRX 종가)를 못 구해 서버가 레거시 방식
//    (실제가로 대체)으로 폴백했다는 뜻이다. 그 경우 값이 정적일 수 있어 표시하지 않는다.
(function(){
