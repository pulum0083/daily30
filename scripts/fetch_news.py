...
- **[임시 2026-07-22 추가, 실적 시즌 종료 시 제거]** 구글(알파벳) 분기 실적 발표 — 지금 빅테크 실적 시즌의 핵심 분기점이므로 **명시적으로 검색**한다. 실적·가이던스, 특히 **클라우드·AI 설비투자(capex) 코멘트**가 한국 반도체·HBM(삼성전자·SK하이닉스) 수요 기대에 미치는 파급(read-through)을 함께 찾는다. **단, 실제로 발표·검색된 경우에만** catalysts·key_indicators에 담고, 발표 전이거나 검색되지 않으면 지어내지 않는다(운영 규칙 0).
