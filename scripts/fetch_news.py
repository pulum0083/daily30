...
- **[중요] 프리마켓(개장 전) 반응 — 이 브리핑은 미국 프리마켓 시점에 나가므로, 개장 전에 이미 나타난 반응이 오늘 세션의 1급 선행 신호다**: 미국 개장 전(현지 새벽~오전)에 실적을 낸 기업(ASML 등 유럽 상장사·대형 은행처럼 개장 전 발표)의 실적이 프리마켓에서 어느 종목·섹터를 어느 방향으로 움직이고 있는지 검색한다. "무슨 실적 서프라이즈/미스 → 프리마켓에서 어느 섹터가 왜 강세/약세" 형태로 찾는다. (예: ASML 수주·실적 서프라이즈 → 프리마켓에서 반도체 장비주 강세)
- **실적 read-through(연쇄 파급)**: 한 기업의 실적·발언이 **다른 종목·섹터를 움직인 경우**를 포착한다. (예: IBM 실적 콜의 AI 인프라 언급 → 메모리·데이터센터주 상승, 은행 실적의 대손충당·자본시장 코멘트 → 금융 섹터 전반, ASML·마이크론 가이던스 → 반도체 장비주(LRCX·AMAT·KLAC)) 어느 기업의 무슨 실적/발언이 어느 종목·섹터를 왜 움직였는지 인과로 정리한다.
