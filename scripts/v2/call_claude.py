...
**[규칙 A] 의견 먼저, 데이터는 뒤에**
첫 문장(또는 첫 절)에 코스피·주식에 어떤 영향이 있는지(의견·결론)를 쓰고, 그 뒤에 왜 그런지(데이터·수치 근거)를 붙인다.
수치는 <b>태그</b>로 최소 1개 강조한다.
❌ 데이터 먼저 금지: "나스닥이 +1.5% 올랐어요. 코스피 IT 섹터에 긍정적이에요."
✅ "코스피 IT 섹터가 강세 출발할 것 같아요. 나스닥이 <b>+1.5%</b> 올랐거든요."

**[규칙 C] 문장은 가능하면 단문으로 쓴다**
한 문장에 여러 정보를 욱여넣지 않는다. 할 말이 충분하면 2문장도 좋지만, 1문장으로 깔끔하게 전달되면 1문장으로 끝낸다.
긴 복문("~했고, ~하며, ~이어서, ~이기 때문에")보다 짧은 단문 2개가 읽기 쉽다.
❌ "나스닥이 +1.5% 올랐고 NQ선물도 강세를 보이며 빅테크가 동반 상승해 코스피 IT 섹터에 긍정적이에요."
✅ "코스피 IT 섹터가 오를 것 같아요. 나스닥 <b>+1.5%</b>에 빅테크도 동반 강세거든요."
...
- 각 항목은 **150자 이내**, 이모지 1개로 시작. 문장 수는 제한 없음 — 단문이 가능하면 1문장, 필요하면 2문장 (규칙 A·C 준수)
...
**[필수 규칙 5] 빅테크 개별 종목·공포탐욕 추세를 적극 활용한다**
reasons를 더 구체적으로 만들기 위해 아래 데이터를 적극 인용한다.
- bigtech (NVDA·AAPL·MSFT·AMZN·META·GOOGL 개별 change_pct): 어젯밤 미국 빅테크 중 **가장 크게 움직인 리더/패자**를 종목명과 수치로 명시한다.
  예: "엔비디아가 <b>+4.2%</b> 급등하며 AI 반도체 투자심리를 끌어올렸거든요." (지수만 말하지 말고 개별 리더를 짚는다)
- fearGreed (value/prev/1w): 현재 값만 말하지 말고 **추세 변화**를 함께 쓴다.
  예: "공포탐욕지수가 지난주 <b>23</b>에서 오늘 <b>28</b>로 회복되며 투자심리가 개선되고 있거든요."

### 반대 시나리오(counter_signal) 작성 규칙
오늘 예측 방향(direction)을 **뒤집을 수 있는 가장 큰 위험 변수 1가지**를 짚는 균형 장치다.
한쪽으로 치우친 브리핑을 막고 신뢰도를 높이기 위해 **항상 1개 작성한다**.
- 정확히 1~2문장, 해요체. 규칙 A(의견 먼저)·규칙 B(해요체) 동일 적용. <b>수치</b> 1개 이상 권장.
- **상승 우위 예측이면** 상승을 꺾을 수 있는 하방 위험을, **하락 우위 예측이면** 반등 가능성이 있는 상방 변수를 쓴다.
  예(상승 예측 시): "다만 오늘 밤 ISM 제조업 PMI가 예상치 <b>53.3</b>을 크게 밑돌면 경기 둔화 우려로 상승분을 반납할 수 있어요."
  예(하락 예측 시): "다만 원/달러 환율이 <b>1,500원</b> 아래로 안정되면 외국인 매도세가 진정되며 낙폭이 줄어들 수 있어요."
- reasons에 이미 쓴 근거를 그대로 반복하지 않는다. 새로운 위험 각도를 제시한다.
- 마땅한 위험 변수가 정말 없으면 빈 문자열 ""로 출력한다 (가급적 1개는 찾아 쓴다).
...

{
  ...
  "counter_signal": "다만 오늘 밤 美 경제지표가 예상을 밑돌면 상승분을 일부 반납할 수 있어요. <b>53.3</b> 예상의 ISM 제조업 PMI가 쇼크로 나오면 경기 둔화 우려가 외국인 매도로 이어질 수 있거든요.",
  "watch_items": [
    {"icon": "🇺🇸", "label": "오늘밤 美 PCE 물가", "text": "예상치 <b>+0.2%</b> 부합 여부가 금리 경로의 분수령이에요."},
    {"icon": "💱", "label": "원/달러 환율", "levels": [{"label": "지지", "num": "1,508", "cls": "dn"}, {"label": "저항", "num": "1,520", "cls": "up"}]}
  ],
  "stock_picks": [
    {
      "name": "종목명",
...
한 문장에 여러 정보를 욱여넣지 않는다. 할 말이 충분하면 2문장도 좋지만, 1문장으로 깔끔하게 전달되면 1문장으로 끝낸다.
긴 복문("~했고, ~하며, ~이어서, ~이기 때문에")보다 짧은 단문 2개가 읽기 쉽다.
❌ "S&P500 선물이 -0.3% 내려가고 나스닥도 약세를 보이며 빅테크 전반이 하락해 오늘 증시가 약하게 열릴 것 같아요."
✅ "오늘 미국 증시가 약하게 시작할 것 같아요. S&P500 선물 <b>-0.3%</b>에 나스닥도 약세거든요."
