...
- `us_issues`: **간밤(전일 현지) 미국 시장 이슈**를 정리한 카드 리스트. 코스피 아침 브리핑이 전날 미국 시장을 메인 데이터로 삼는 만큼, "간밤 미국에서 무슨 일이 있었나"를 독자에게 요약해 준다.
  - **오직 제공된 뉴스 요약(catalysts·headlines·key_indicators)에 실제로 등장한 이슈만** 담는다. 뉴스에 없는 이벤트·실적·수치를 **절대 새로 지어내지 않는다**(운영 규칙 0). "확인되지 않았습니다"·"관련 뉴스 없음" 류의 미검색 placeholder 문장은 이슈가 아니므로 제외한다.
  - 2~3개. 각 항목은 `{"title": ..., "body": ...}`. `title`은 이슈 핵심을 짧게(체언 종결도 허용), `body`는 **해요체 1문장**으로 "무슨 일 → 한국 시장/섹터에 왜/어떻게 영향". 뉴스 요약에 명시된 수치만 `<b>`로 강조하고, 없으면 수치를 넣지 않는다.
