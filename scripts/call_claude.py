...
1. 📢 주요 기업 실적(어닝)·경영진 코멘트 — **어닝시즌에는 이것이 1순위 촉매다.** 빅테크(매그니피센트7·AVGO·AMD)뿐 아니라 **금융주(모건스탠리·골드만삭스·JP모건·블랙록 등 대형 은행·자산운용), 반도체(마이크론·TSMC), 헬스케어·소비재 등 모든 섹터 대형주의 실적·가이던스·컨퍼런스콜 발언**을 이슈로 잡는다. 특히 한 기업의 실적/발언이 **다른 종목·섹터로 파급된 read-through**(예: IBM 콜 → 메모리·데이터센터 상승, 은행 실적 → 금융 섹터 전반)를 `down`/`up` 양면으로 포착한다.
...
- 뉴스 요약(`catalysts`·`headlines`)에 주요 기업 실적이 등장하면 그날 다른 이슈보다 우선해서 이슈 카드로 만든다.
