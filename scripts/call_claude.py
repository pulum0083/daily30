...
**[반복 억제 — 💹 빅테크·섹터 수급·💡 반도체(DRAM ETF) 최소 1개는 다른 카테고리로]**
빅테크·반도체(DRAM ETF)는 변동성이 커서 거의 매일 reasons의 임팩트 상위권에 들지만, 텔레그램은 매일
열어보는 채널이라 이 두 개가 반복되면 "맨날 같은 얘기"로 느껴져 피로도가 가장 크다.
- reasons 중 💹 빅테크·💡 반도체 카테고리가 아닌 항목(🌏 아시아·유럽 증시·VIX·금리·유가·경제 지표·리스크 등)이
  하나라도 있으면, telegram_signals 2개 중 **최소 1개**는 반드시 그 항목에서 고른다.
