...
- `us_issues`: **간밤(전일 현지) 미국 시장 이슈**를 정리한 카드 리스트. 코스피 아침 브리핑이 전날 미국 시장을 메인 데이터로 삼는 만큼, "간밤 미국에서 무슨 일이 있었나"를 독자에게 요약해 준다.
  ...
  - **간밤 미국 대형주 실적(특히 구글/알파벳 등 빅테크)이 뉴스 요약에 있으면 us_issues 최우선으로 담는다** — 실적·클라우드/AI capex 코멘트가 한국 반도체·HBM 수요에 주는 파급을 한 문장으로. (단, 뉴스 요약에 실제로 있을 때만. 없으면 억지로 넣지 않는다 — 운영 규칙 0.)
