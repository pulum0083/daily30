...
- 💹 빅테크·섹터 수급 — 매그니피센트7(NVDA·AAPL·MSFT·GOOGL·AMZN·META·TSLA) + 브로드컴(AVGO)·AMD·스페이스X(SPCX, 2026-06 상장·xAI 합병) 등 시총 상위 대형주 큰 등락(±2% 이상). 가능하면 뉴스 촉매를 함께 엮는다.
- 💡 반도체·DRAM ETF — SOX 또는 DRAM ETF 등락이 ±2% 이상. 가능하면 뉴스 촉매를 함께 엮는다.

위 시그널 풀 중 오늘 데이터에서 변동이 없거나 영향이 미미한 항목은 선택하지 않는다.

**[반복 억제 — 💹 빅테크·섹터 수급·💡 반도체(DRAM ETF)]**
이 두 카테고리는 매그니피센트7·SOX·DRAM ETF가 거의 매일 ±2% 안팎으로 움직여 조건을 쉽게 충족하고, 그래서
매일 같은 이야기가 반복되기 쉽다. 가장 확실한 근거이지만 매번 등장하면 정보 가치가 떨어진다.
- 위 임계치(±2%)를 눈에 띄게 넘는 '오늘의 주인공' 수준(예: ±4% 이상, 또는 뚜렷한 뉴스 촉매 동반)일 때만
  reasons[1]~[3]에 **독립 항목**으로 쓴다.
- 그 미만의 평범한 등락이면 독립 항목으로 만들지 말고, 다른 카테고리(아시아·유럽 증시·VIX·금리·유가·경제 지표·
  리스크)에서 오늘 더 임팩트 있는 걸 우선 선택한다.
- "빅테크가 강세예요", "SOX·DRAM ETF가 올랐어요" 같은 동일 문형을 매일 반복하지 않는다. 등장하더라도 표현을 매번 바꾼다.
...
**[이슈 우선 — 💹 빅테크·섹터 수급·💡 반도체(DRAM ETF) 최소 1개는 다른 카테고리로]**
빅테크·반도체(DRAM ETF)는 변동성이 커서 거의 매일 reasons의 임팩트 상위권에 들지만, 단순 등락 수치일 뿐
이슈가 아니라서 텔레그램(매일 열어보는 채널)에서 반복되면 "맨날 같은 얘기"로 느껴져 피로도가 가장 크다.
- reasons 중 📢 실적·이슈처럼 구체적 뉴스 촉매가 있는 항목이 하나라도 있으면 **최우선으로** 선택한다.
  그 다음 🌏 아시아·유럽 증시·VIX·금리·유가·경제 지표·리스크 등이 하나라도 있으면, telegram_signals 2개 중
  **최소 1개**는 반드시 그 항목에서 고른다. 단순 ETF·지수 등락 수치만 있는 💹 빅테크·💡 반도체는 이런
  이슈성 항목이 없을 때만 채택한다.
- 💹 빅테크·💡 반도체 두 카테고리를 함께 telegram_signals에 넣는 것은, 오늘 다른 카테고리에 쓸 만한 항목이
  정말 하나도 없을 때만 예외로 허용한다.
- 두 카테고리 중 하나를 넣더라도, 전날과 똑같은 문형("빅테크가 강세예요" 등)을 반복하지 말고 표현을 바꾼다.
...

{
  ...
  "telegram_signals": [
    ...
    "🏦 어젯밤 CPI가 예상치를 밑돌며 금리 인하 기대가 커졌어요. 국채 금리도 <b>-5bp</b> 내려갔어요."
  ]
}

(위 telegram_signals 예시는 형식 참고용이다. 실제로는 그날 데이터에서 임팩트가 큰 걸 고르되,
빅테크·반도체 두 카테고리로만 채우지 않는다 — 위 반복 억제 규칙을 따른다.)
...
- `num_cards`: 정확히 3~4개. 각 항목은 `{"label": ..., "value": ..., "dir": ..., "caption": ...}` 객체. **오늘을 규정하는 핵심 수치**만 고른다.
  ...
  - `caption`: 그 수치의 의미 한 줄. 완결된 해요체, 30자 안팎, <b> 강조 가능.
  - **⚠️ num_cards 중 최소 1개는 순수 지수·거시 수치가 아니라 오늘 뉴스 요약(catalysts·headlines)에 실제로 등장한 구체적 사건**(실적 서프라이즈·M&A·신제품·정책 발표 등)을 카드로 만든다. label엔 사건 주체(예: "엔비디아 실적"), value엔 그 사건과 직결된 수치가 있으면 쓰고 없으면 상태어(예: "발표"), caption엔 사건 내용을 담는다. 뉴스 요약에 미국 증시에 영향을 줄 만한 구체 사건이 없으면 이 규칙은 건너뛰고 수치 카드로만 채운다 — 억지로 지어내지 않는다.
- `num_take`: 카드들을 종합한 결론 1~2문장 (해요체, 오늘 방향 + 관전 포인트).
`reasons` 필드는 출력하지 않는다.
"""
