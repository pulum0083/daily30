...
- `key_drivers`: 정확히 3개. 각 항목은 `{"text": ..., "codes": [...]}` 객체.
  ...
  - **간밤 미국 빅테크 실적은 코스피 대장주와 직결되는 최우선 촉매다.** 빅테크(구글·MS·아마존·메타·엔비디아 등)의 **AI capex 성장률**이 메모리·HBM 수요의 선행지표이기 때문 — capex가 늘면 삼성전자·SK하이닉스 HBM 수요 기대↑(코스피 상방), **capex 성장률이 꺾이면 메모리 수요 둔화 우려로 대장주·코스피 하방**으로 직결된다. 빅테크 실적이 뉴스 요약에 있으면 `key_drivers[0]`과 `todays_view`에서 이 파급을 **최우선 동인**으로 앞세운다(실적·클라우드/AI capex → HBM read-through를 한 문장으로). 단, 뉴스 요약에 실제로 있을 때만 — 없으면 억지로 넣지 않는다(운영 규칙 0).
  - **이미 발표된 실적이면 '발표 예정'이 아니라 '결과'로 쓴다.** 미국 실적은 미 증시 마감 후(이 브리핑 직전 KST 새벽)에 나오므로 이 시점엔 보통 결과가 나와 있다. 뉴스 요약에 결과 수치(매출 증감률·클라우드 성장률·capex 규모·발표 후 주가 반응)가 있으면 그 **결과**를 담고, "오늘 밤 발표 예정"·"실적 대기" 같은 미래형 프레이밍으로 쓰지 않는다. reason_title·todays_view·key_drivers 모두 결과 기준으로 서술한다.
