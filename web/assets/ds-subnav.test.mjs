...
test('탭 정의는 이번 범위인 2개만 점등한다', () => {
  ...
  assert.deepEqual(Array.from(TABS.map((t) => t.id)), ['home', 'signals']);
  assert.deepEqual(Array.from(TABS.map((t) => t.label)), ['전체', '특이신호']);
});
...
test('렌더 — 활성 탭에만 is-active와 aria-current가 붙는다', () => {
  const host = mkHost();
  load({ host, pathname: '/stocks/', hash: '#signals-all' });

  assert.ok(host.innerHTML.includes('data-tab="signals"'));
  const sectorTag = host.innerHTML.match(/<a[^>]*data-tab="signals"[^>]*>/)[0];
...
test('클릭 — /stocks/ 내부 화면은 go()로 전환하고 기본 이동을 막는다', () => {
  ...
  let prevented = false;
  host._click({
    target: { closest: () => ({ getAttribute: () => 'signals' }) },
    preventDefault: () => { prevented = true; },
  });

  assert.deepEqual(calls, ['signals-all']);
  assert.equal(prevented, true);
});

test('클릭 — go()가 없는 페이지에서는 가로채지 않는다', () => {
  // /themes/에서 "특이신호" 탭을 누르면 /stocks/#signals-all으로 정상 이동한 뒤
...
test('dsSubnavSync는 현재 위치로 강조를 다시 계산한다', () => {
  ...
  win.location.hash = '#signals-all';       // go()가 해시를 바꾼 뒤의 상태를 흉내낸다
  win.dsSubnavSync();

  assert.ok(host.innerHTML.match(/<a[^>]*data-tab="signals"[^>]*>/)[0].includes('is-active'));
  assert.ok(!host.innerHTML.match(/<a[^>]*data-tab="home"[^>]*>/)[0].includes('is-active'));
});
...
test('같은 window에 스크립트가 두 번 실행돼도 리스너가 중복 등록되지 않는다 (더블 로드)', () => {
  ...
  assert.deepEqual(calls, ['signals-all'], '클릭 한 번에 go()가 정확히 한 번만 호출돼야 한다');
});
