...
|---|---|---|---|
| **코스피 시초가** | 월~금 07:30 | 한국 시장 개장일 | 전일 미국 마감 기반 코스피 시초가 방향 예측 + 오늘의 섹터 심층(8개 풀 하이브리드 로테이션) |
| **코스피 마감** | 월~금 16:00 | 한국 시장 개장일 | 장중 흐름·섹터 성과·급등주 분석 + 뉴스 요약 |
...
| **중복 방지** | `telegram_sent_log.json`으로 발송 이력 관리 + trigger.mjs 중복 dispatch 차단 |
...
**노출 조건:** URL 날짜 = 오늘 KST + 평일 + 08:50 이후. **당일 브리핑에서만 표시.** 과거 브리핑 조회 시 표시하지 않는다.  
매 1시간 Gemini가 뉴스를 수집(`fetch_news_live.py`, GitHub Actions `kospi-news-live` 잡)해 스코어보드에 표시한다.

### 예측 섹션 표시 원칙

예측 섹션(방향 게이지·신뢰도·근거)은 **항상 펼쳐진 상태**로 표시한다. 아코디언 토글 없음.

### 게이지 색상 원칙

투자 방향성 색상(`빨강=상승`, `파랑=하락`)과 구분되는 별도 체계를 사용한다.
...
| **요소** | **구현** |
|---|---|
| 라이브 스코어보드 | 예측 vs 실제 실시간 추적 카드 (당일 브리핑 전용) |
| Sparkline 차트 | Canvas API, 최근 10개 데이터 포인트 면적 그래프 |
| 예측 정확도 게이지 | 이탈(회색)~적중(초록) 3구간 바 + 바늘 |
...
| `/briefings/{YYYY-MM-DD}/kospi/` | 코스피 시초가 예측 브리핑 |
| `/briefings/{YYYY-MM-DD}/close/` | 코스피 마감 브리핑 |
| `/briefings/{YYYY-MM-DD}/us/` | 미국 시장 예측 브리핑 |
| `/briefings/ko/{date}/` | 코스피 브리핑 (레거시 호환) |
| `/briefings/us/{date}/` | 미국 브리핑 (레거시 호환) |
| `/briefings/ko-close/{date}/` | 코스피 마감 시황 (레거시 호환) |
...
| `/api/trigger` CRON_SECRET 인증 + method 체크 | 구현 완료 | 5/20 |
| workflow concurrency group (동일 type 동시 실행 방지) | 구현 완료 | 5/20 |
| 코스피 마감 브리핑 뉴스 수집 추가 (Gemini) | 구현 완료 | 5/20 |
| 10년물 금리 필수 규칙 제거 → 시그널 풀 동적 선택 | 적용 완료 | 5/20 |
| 브리핑 index 템플릿 GNB·사이드바 통일 (Chip-Board, AI 반도체 위젯) | 적용 완료 | 5/20 |
| 라이브 스코어보드 (예측 vs 실제 실시간 추적, 1시간 뉴스 갱신) | 적용 완료 | 6/3 |
| 예측 아코디언 토글 제거 (항상 펼침) | 적용 완료 | 6/4 |
| 게이지 색상 개선 (이탈=회색·적중=초록, 투자 방향성과 분리) | 적용 완료 | 6/4 |
| 스코어보드 당일 브리핑 전용 노출 (URL 날짜 체크) | 적용 완료 | 6/4 |
| 휴장일 브리핑 페이지 삭제 처리 (briefings-list.json pending 초기화) | 적용 완료 | 6/4 |
| 예측 결과 위젯 타이틀 구조 개선 (대표=결과 요약·서브=등락률 분리, hit.dn 아쉬움 표현, 문구 다양화) | 적용 완료 | 6/4 |
...
| ~~Mid~~ | ~~주간 리포트 자동화~~ | ~~주간 요약 + 다음주 이벤트 캘린더~~ | ❌ 폐기 |
...
│   ├── news_summary_kospi.json        ← Gemini 뉴스 요약 (시초가)
│   ├── news_summary_kospi-close.json  ← Gemini 뉴스 요약 (마감)
│   └── news_summary_us.json           ← Gemini 뉴스 요약 (미국)
├── docs/
│   ├── PRD.md                         ← 본 문서
...
| 2026-06-03 | **데이터 정합성 전면 강화:** ① 종목 픽 실측 주입(검증 게이트, 미국 yfinance·한국 네이버 일봉) ② 한국 KOSDAQ `.KS` 유령데이터 버그 수정 ③ 파이프라인 재배치(검증→웹페이지→텔레그램) ④ 프리장 신고가·낙수 섹터% 날조 수치 제거 ⑤ 뉴스 수집 RSS→Gemini Google Search grounding 교체 **+ 라이브 스코어보드 구현** (예측 vs 실제 실시간 추적, 매 1시간 Gemini 뉴스 갱신) |
| 2026-06-04 | **UI 정비 및 스코어보드 노출 규칙 확립:** ① 예측 아코디언 토글 제거(항상 펼침) ② 게이지 색상 개선(이탈=회색·적중=초록) ③ 스코어보드 당일 브리핑 전용 노출(URL 날짜 체크) ④ 현충일 휴장으로 06-03 코스피 페이지 삭제, briefings-list.json 초기화 ⑤ 서비스 운영 규칙 문서 작성(SERVICE_RULES.md) |
| 2026-06-04 (2차) | **라이브 스코어보드 전면 개선:** ① pred-tag 색상 방향 고정(dn=파란/up=빨간) ② 코멘트 방향별 풀 분리(상승/하락 각 hit·miss 4세트, 랜덤 선택) ③ 코스피 지수 오도미터 효과(자릿수별 롤링) ④ 카운트다운 슬라이드업 애니메이션 ⑤ 뉴스 수집 7회/일 스케줄 확장(09:10~15:00 KST) ⑥ 지수 갱신 주기 30초→10초(s-maxage=10) ⑦ kospi-news-live 워크플로우 checkout 토큰 버그 수정 ⑧ 배포 규칙 확립(push=배포, 명시적 요청 시만) |
| 2026-06-04 (3차) | **예측 결과 위젯 타이틀 구조 개선:** ① 대표 타이틀(`lsb-head-em`)에 예측 결과 요약 표시 ("오늘 장이 종료됐어요." 고정 문구 제거) ② 서브 타이틀(`lsb-sub`)은 등락률 한 줄("nn% 하락 마감이에요.") ③ hit.dn(하락 예측 적중) 대표 타이틀에 아쉬움 표현 추가 ④ 케이스별 문구 다양화(hit.dn 8종·hit.up 6종·miss 각 6~7종·tight 5종) ⑤ CLOSE_MSGS 구조 `title`/`sub` 배열 분리 관리 |
