...
```
본문:  오늘의 관점 → 오늘의 이슈(3~5) → 월가 코멘트 → 종목 픽
사이드바:  시장 지표(market_data) → 텔레그램/월배당 CTA
```

**제거:** 방향 예측(prediction) · 이렇게 보는 이유(reasons 산문) · 성적표(사이드바 accuracy).
**유지:** 월가 코멘트(analyst_quotes) · 종목 픽(stock_picks) · 시장 지표.

> **프리장 신고가(`nh_stock`)는 이번 범위에서 제외한다.** 확인 결과 이 섹션은 config-driven 파이프라인에 배선된 적이 없어(고아 템플릿) 현재 라이브에도 표시되지 않으며, 프리장 52주 신고가 실측 데이터를 수집하는 소스도 없다. 운영 규칙 0상 LLM이 생성할 수 없으므로, 실측 수집 단계를 갖춘 별도 기능으로 후속 처리한다.
...
- `todays_view`(오늘의 관점)도 특정 가격·등락률 수치를 넣지 않고 정성적 프레이밍만 쓴다(실측 보장이 없으므로). 단어 강조(`<b>`)는 허용, 숫자 강조는 금지.
