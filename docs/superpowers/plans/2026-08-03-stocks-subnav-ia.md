...
6. 홈에서 "특이 신호 전체 보기 →" 링크(`web/stocks/index.html:210`, `class="more"`) 클릭 → `#signals-all`로 전환되며 **서브탭의 "특이신호"가 활성으로 바뀐다**(이 링크는 서브탭 자체가 아니라 기존 인페이지 링크인데, `go()` 훅을 타므로 서브탭도 같이 갱신되는지 확인하는 것이 이 스텝의 목적이다). ⚠️ "패시브 민감주" 링크는 아직 홈에 없다(Task 6에서 추가) — 여기서 시도하지 말 것.
