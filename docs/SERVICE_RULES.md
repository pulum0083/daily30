...
| 종목    | 1순위           | 2순위 (폴백) |
| ----- | ------------- | ------------ |
| 한국 종목 | 네이버 일봉 — 15:30 뒤 오늘 봉은 15:30 1분봉으로 교정 (`kr_official_closes.py`) | 없음 — 실패 시 비운다. **토스 일봉은 공식 종가가 아니라 쓰지 않는다(§48)** |
...
- **타임라인 형식이고, 타이틀이 주인공이다**(2026-09-03). 좌측 레일에 날짜 노드를 꿰고 각
  ...
  83편이 전부 같은 라벨의 반복이라 정작 읽을 타이틀이 묻혔다 — **태그를 강조하지 말 것.**
- **상단 설명은 한 줄만 남기고 `<details>`로 기본 접힘**(2026-09-03) — 섹터 페이지와 같은 규칙.
  접혀 있어도 크롤러가 내용과 `/about/`·`/legal/disclaimer/` 링크를 읽으므로 본문·내부 링크에
  손해가 없다(측정: 4,042자 → 4,052자).
- **레일·마커 정렬은 계산으로 맞춘다**(2026-09-03 정정). 마커 중심을 레일 중심과 첫 줄 텍스트
  중심에 동시에 맞추려면 두 값이 고정이어야 해서 라벨에 `line-height`를 명시했다 — 기본
  line-height에 기대면 폰트가 바뀔 때 다시 어긋난다. 가로는 `마커 left = 레일 중심 - 지름/2`,
  세로는 `마커 top = (line-height - 지름) / 2`. 데스크톱·모바일 각각 4개 축을 브라우저에서
  실측해 오차 0px을 확인했다(수정 전엔 가로 1px·세로 3.25px 어긋나 있었다).
...
### 31. 6월 24일 마이크론 실적이 8월 4일 브리핑의 1번 이슈로 — 방관자 티커가 어닝 게이트를 무력화 (2026-08-04 실사고, 수정 완료)

**증상**: 2026-08-04 21:17 KST 미국 브리핑의 관점·1번 이슈·픽 시나리오가 전부 "마이크론 어닝 서프라이즈"였다. 마이크론의 실제 직전 실적 발표일은 **2026-06-24로 41일 전**이고, 다음 발표는 9/23이다. 같은 브리핑에 "테슬라 신규 배터리 기술 발표"(미검증)와 "7월 비농업 고용 예상치 하회"(**실제 발표 예정일 2026-08-07**, 그날 발표된 적 없음)도 실렸다. `catalyst_history_us.json` 확인 결과 같은 고용지표 날조가 **7/31·8/3·8/4 사흘 연속** 실렸다.

**근본 원인 — 어닝 게이트(§22)의 "방관자 티커" 구멍**:

`_drop_stale_earnings`는 "여러 티커 중 **하나라도** 최근(≤2일) 발표면 신선한 실적으로 보고 유지"한다. 문제의 catalyst는 이 형태였다.

```json
{"text": "마이크론(MU)의 3분기 실적 예상치 상회 → 프리마켓에서 반도체 관련주(AMD, QCOM) 강세",
 "ticker": "MU,AMD,QCOM"}
```

실측 나이는 **MU 41일 / AMD 0일 / QCOM 6일**이었다 — **AMD가 하필 그날(8/4) 실적을 발표**했다. `min(ages)=0 ≤ 2`라 게이트는 이 항목을 "신선한 실적"으로 판정해 통과시켰다. 즉 **주장의 주어는 41일 된 마이크론인데, 영향 절에 이름만 얹힌 방관자(AMD)가 그날 실적을 냈다는 이유로 stale 주장이 무임승차**했다.

결정적 대조 증거: **같은 날조가 headlines에서는 정상 제외됐다**("마이크론, 예상을 뛰어넘는 실적 발표…반도체 섹터 훈풍 예고" → `41일 전 > 2` 로그). headlines는 순수 문자열이라 ticker 필드가 없어 `마이크론→MU` 하나만 잡혔기 때문이다. **ticker 필드에 영향 종목까지 섞여 들어온 것이 게이트를 무력화한 것**이다.

**함께 드러난 세 가지 (전부 이번에 수정)**:

1. **중복 방지 장치가 날조 급식기로 작동**. `_load_prev_catalysts`가 "이미 다룬 주제"로 프롬프트에 주입한 직전 실행 목록을, Gemini가 **오늘의 catalysts로 그대로 복사**해 돌려줬다(재현 확인 — 각 항목 끝에 "(참고: 직전 브리핑에서 언급된 내용으로…)"라고 스스로 주석까지 달았다). 어제 실린 거짓이 오늘 프롬프트에 들어가 오늘 다시 실린다. §28의 '프롬프트에 방치된 임시 힌트'와 같은 계열인데, 이쪽은 **매일 자동으로 주입**되므로 더 나쁘다.
2. **`catalysts_schema_violation` 신호가 정보량 0이었다.** `_grounding_failure_signals`가 `_filter_stale_catalysts`가 항목을 문자열로 환원한 **뒤에** 호출돼, catalysts가 비어있지만 않으면 **항상** 발화했다. §29가 만든 "신호 2개면 폐기" 임계가 사실상 "다른 신호 1개면 폐기"로 미끄러져 있었고, 정작 원본 스키마 위반은 아무도 못 봤다.
3. **모든 내용 게이트가 `c["text"]`만 읽었다.** `_filter_stale_catalysts`만 `c["catalyst"]` 폴백이 있었다. 즉 모델이 본문 키 이름을 바꾸면 게이트는 빈 문자열을 보고 전부 통과시키는데 최종 출력에는 본문이 실린다 — 이번 사고의 원인은 아니었지만 **JSON 키 이름 하나로 전 게이트가 우회되는** 잠복 구멍이었다.

**측정으로 확인한 진짜 뿌리 — 뉴스 수집에 출처가 없다**:

`fetch_news.py`는 `google_search` 툴로 "LLM에게 검색시켜 사실을 JSON으로 받는" 구조인데, 응답에 **출처가 하나도 붙지 않는다**(실측).

| 모델 | 검색 쿼리 | grounding_chunks(출처) |
| --- | --- | --- |
| gemini-2.5-flash-lite (현행) | 2건 발행 | **0건** |
| gemini-3.6-flash | 없음 (grounding_metadata 자체가 None) | — |
| gemini-pro-latest | 없음 (동일) | — |

프로덕션 증거도 같다 — `data/analyst_quotes.json` 4건이 **전부 URL 없음**이다(§11이 "URL은 반드시 grounding_chunks에서 추출한다"고 못 박아둔 그 경로가 죽어 있다). 반면 **RSS로 직접 수집하는** `fetch_ib_korea_views.json`은 2건 전부 실제 원문 URL을 갖는다. 같은 저장소 안에 대조군이 있고, 차이는 모델이 아니라 **아키텍처**다.

즉 §22·§24·§25·§27·§28·§29에서 여덟 번 게이트를 덧댔지만 전부 "생성된 거짓을 사후에 반박"하는 방식이었고, 근거 문서가 애초에 존재하지 않으니 반박할 수 있는 것만 걸러지고 나머지는 통과한다. 이번엔 **실적 발표일이라는 실측 대조가 있었는데도** 방관자 티커 하나로 뚫렸다.

**수정**:
- `_earnings_subject_tickers()` 신설 — catalyst 형식이 "사건 → 영향"이므로 **화살표 왼쪽(사건 주어)의 티커만** 검증한다. 산문에서 티커는 괄호 안에 오므로 괄호 티커 + 사명 resolve만 쓴다(맨 문자열 스캔은 "JP모건"의 "JP"를 티커로 오인한다). 화살표가 없어 주어를 특정할 수 없으면 기존 동작으로 fail-open.
- `_drop_prev_run_echoes()` 신설 — 프롬프트에 주입한 직전 실행 목록을 되뱉은 항목을 제거(문자 3-gram 유사도 + 자기 라벨 "직전 브리핑에서 언급").
- `_item_text()`를 **키 이름에 의존하지 않게** 고치고(알려진 키 → 없으면 값에서 본문 추정), `_normalize_news_items()`로 게이트 **이전에** 항목 모양을 통일. 모든 내용 게이트가 같은 본문을 본다. 출력 shape은 기존대로 문자열로 되돌려 다운스트림 계약을 유지한다.
- `_grounding_failure_signals()`를 **정규화·게이트 이전의 원본**으로 호출하도록 이동. 스키마 신호를 "전부 문자열 **또는** dict인데 text 없음"으로 실질화.
- `_count_grounding_chunks()` 신설 — 응답에 붙은 출처 수를 세어 `no_grounding_sources` 신호로 삼고 매 실행 로그에 남긴다(현재 상시 0건이라는 사실이 로그로 드러나게 한다).
- 테스트 34건 추가(`test_earnings_subject.py` 14, `test_news_item_normalize.py` 20). 실사고 데이터 리플레이 포함 — MU 41일 + AMD 0일 조합에서 실제로 제외되고, 진짜 신선한 실적·다중 주어는 살아남는 것까지 검증. 전체 593건 통과.

**방지 룰**:
- **주장의 주어와 영향 대상을 구분해 검증한다.** "A 때문에 B가 움직였다"에서 검증 대상은 **A**다. 등장 티커를 뭉뚱그려 "하나라도 최신이면 통과"시키면, 오늘 실적을 낸 아무 종목이나 영향 절에 끼워 넣는 것으로 어떤 stale 주장도 통과시킬 수 있다. 관대한 방향(`min`)으로 뭉개는 집계는 **게이트를 조건부로 꺼버리는 스위치**와 같다.
- **중복 방지용으로 모델에 보여준 목록은 모델이 답으로 돌려줄 수 있다.** "이건 하지 마"로 넣은 텍스트는 곧 "이런 걸 쓰면 된다"는 예시이기도 하다. 부정 예시를 프롬프트에 주입하면 **반드시 출력에서 그 에코를 걸러내는 게이트를 함께 둔다**(§25 슬롯 예시, §28 임시 힌트, 이번 중복 방지 목록 — 세 번째 재발이다).
- **신호를 측정하는 지점이 게이트 앞인지 뒤인지 확인한다.** 게이트가 지운 뒤에 재면 그 신호는 영원히 안 뜨거나(플레이스홀더) 영원히 뜬다(스키마). 둘 다 정보량 0이고, 임계 기반 판정 전체를 조용히 왜곡한다.
- **"출처가 몇 건 붙었는가"를 매 수집마다 로그에 남긴다.** 0건이면 그 답은 어떤 문서로도 뒷받침되지 않는다. 이 한 줄이 없어서 "검색해서 가져온다"는 전제가 3개월간 검증되지 않은 채 유지됐다.
...
**동반 발견 — 중복 발송 가드가 한 달간 죽어 있었다 (같은 날 수정 완료)**: `data/telegram_sent_log.json`의 "하루 1회 발송 제한"이 작동하지 않고 있었다. 로그는 **2026-07-02에 멈춰** 있었고 `already_sent_today()`는 항상 False를 반환했다.

원인은 이번 사고의 원인과 같은 뿌리다. 2026-07-02 16:46 커밋 [b257bdc9](https://github.com/pulum0083/daily30/commit/b257bdc9)가 §17 대응으로 텔레그램을 커밋 스텝 **뒤**로 옮겼는데(페이지가 라이브인 걸 확인한 뒤 보내야 하므로 이 순서 자체는 옳다), 그 전까지는 텔레그램이 커밋보다 앞이라 `git add data/`가 `mark_sent_today()`의 기록을 함께 커밋하고 있었다. 마지막 자동 커밋이 그날 16:33 마감 브리핑인 것과 정확히 일치한다. **한 스텝의 위치를 옮겼더니 다른 기능이 조용히 죽었고, 그 기능의 실패 모드가 fail-open이라 한 달간 아무 증상이 없었다.**

- 순서를 되돌릴 수 없으므로, 전송 직후 **이 한 파일만** 커밋하는 스텝([commit_sent_log.sh](../.github/scripts/commit_sent_log.sh))을 3개 브리핑 잡에 추가했다. `rebase --autostash`로 다른 스텝이 남긴 미스테이지 변경과 충돌하지 않게 한다(§18). 실패해도 잡을 되돌리지 않는다 — 기록 유실은 발행 실패가 아니다.
- `_warn_if_guard_is_dead()` — 기록이 7일 넘게 멈춰 있으면 경고한다. **발송은 막지 않는다**(경고 때문에 브리핑이 끊기면 본말전도). 위 커밋 스텝이 실패했을 때 그 사실이 다음 실행에서 드러나게 하는 장치이자, 이번 같은 무음 사망의 재발 감지기다. §20 `_targets_data_is_stale()`과 같은 처방이다.
- 테스트 `scripts/test_sent_log_staleness.py`(6).

- **방지 룰(fail-open 가드는 스스로 살아 있음을 증명해야 한다)**: "막는" 것이 일인 코드는 죽어도 아무 증상이 없다 — 오히려 모든 게 잘 통과하므로 정상처럼 보인다. 차단 가드를 만들 때는 **가드 자신이 최근에 작동했는지**를 함께 관측하고, 오래 조용하면 경고한다. §20이 "실측 데이터 소스에 신선도 가드를 둔다"였다면, 이건 **가드에 신선도 가드를 두는 것**이다.
- **방지 룰(스텝 순서를 바꿀 때 그 스텝의 부수 효과를 목록화한다)**: `git add web/ data/`처럼 넓은 경로를 커밋하는 스텝은 **다른 스텝이 만든 파일까지 함께 보존하는 숨은 계약**을 맺고 있다. 스텝 순서를 바꾸면 그 계약이 조용히 깨진다(§18은 같은 넓은 `git add`가 *충돌*을 일으킨 사례였고, 이번은 반대로 *보존이 끊긴* 사례다). 순서를 바꿀 때는 "이 스텝이 쓰는 파일 중 커밋돼야 하는 게 있나"를 확인한다.
...
### 33. 발행된 브리핑은 삭제하지 않는다 (2026-08-11 결정)

`scripts/prune_briefings.py`(`KEEP_DAYS = 15`)가 3개 브리핑 잡에서 매 거래일 실행되며 15일 넘은 `web/briefings/{date}/` 디렉터리를 `shutil.rmtree`로 지우고 있었다. **스크립트와 3개 워크플로우 스텝을 모두 제거했다 — 앞으로 발행된 브리핑은 영구 보존한다.**

**왜 지우면 안 됐나**: `generate_html.write_sitemap_xml()`이 브리핑 URL을 `sitemap.xml`에 등재하는데(제거 시점 35개), 15일 뒤 그 페이지가 사라져 **검색엔진에 등재해놓고 404를 주는 상태**가 상시 유지됐다. `robots.txt`는 `Allow: /`이고 `base.html:11`도 `index, follow`라 막는 장치가 없었다.

**삭제가 아끼던 것은 git 용량이 아니다.** 브리핑 HTML은 한 번 쓰고 수정되지 않는 파일이라, 삭제해도 blob은 히스토리에 영구히 남는다(현재 `.git` 80MB에 이미 지워진 브리핑들이 포함돼 있다). 즉 prune이 줄이던 것은 체크아웃·배포 크기뿐이었고, 그 규모는 날짜당 약 138KB · 연 34MB로 대가에 비해 미미했다.

- **방지 룰(삭제 로직과 사이트맵은 같은 수명을 가져야 한다)**: 정적 페이지를 지우는 로직을 넣을 때는 **그 URL을 광고하는 곳(sitemap·목록 JSON·내부 링크)도 함께 지우는지** 확인한다. 한쪽만 있으면 404를 능동적으로 생산한다. 반대로 사이트맵에 URL을 추가할 때는 "이 파일이 언젠가 지워지나"를 확인한다.
- **방지 룰(용량 절감 주장은 측정으로 확인한다)**: "오래된 파일을 지워 저장소를 관리한다"는 직관은 **git에서 대체로 틀리다** — 삭제는 히스토리를 줄이지 않는다. 절감 대상이 워킹트리인지 히스토리인지 배포 산출물인지 먼저 구분하고, `du`로 실제 규모를 재고 나서 판단한다.
- **알려진 남은 갭**: 브리핑 목록 UI는 `build_list_context`의 `past_dates[:10]`으로 여전히 최근 10일만 보여준다(§4). 보존된 과거 브리핑은 사이트맵과 직접 URL로만 접근된다 — 목록에서 더 보여줄지는 별도 판단.
- **미복구분**: 2026-06-01 ~ 07-21의 78개 페이지(37개 날짜)가 이 결정 이전에 삭제됐고 git 히스토리에 남아 있다. 복구하려면 그 시기 콘텐츠가 §0 기준을 만족하는지 개별 검증이 필요해 이번 변경에는 포함하지 않았다.

### 34. 형식은 강제로 찍는데 그 형식의 본문이 있는지는 아무도 확인하지 않았다 — 빈 껍데기 섹션 발행 (2026-08-13 실사고, 수정 완료)

**증상**: 2026-08-13 07:27 KST 코스피 예측 브리핑의 근거 섹션이 **통째로 비어서** 나갔다. 발행된 HTML에 `<div class="keynum-grid">` 안이 빈 채로 그대로 들어 있었다(전일 08-12는 같은 자리에 수치 카드 4장 + 종합 결론). 사용자가 "상세 데이터 정보가 없다"고 발견했다.

**근본 원인**: `call_claude`는 그날의 근거 섹션 형식을 랜덤 추첨(`FORMAT_POOL_KOSPI`)해 프롬프트로 지시한 뒤, **응답 내용과 무관하게** 형식을 덮어쓴다.

```python
if chosen_format:
    analysis["analysis_format"] = chosen_format   # 모델이 그 형식으로 답했는지는 확인하지 않는다
```

그날 추첨된 형식은 `keynum`(수치 카드가 본문인 형식)이었는데, 모델은 `num_cards`를 만들지 않고 `split` 형식 모양(`todays_view` + `key_drivers`)으로 응답했다. 그런데도 `keynum`이 찍혔고, `generate_html`이 `keynum` 템플릿을 골라 빈 그리드를 렌더했다.

**피해가 두 배가 된 이유**: `todays_view.html`은 recap/outlook 2단을 **`analysis_format == 'split'`일 때만** 렌더하고, `kospi.html`은 **split이 아닐 때만** 형식별 템플릿을 렌더한다. 즉 형식이 어긋나면 **양쪽 다 빠진다** — 모델이 실제로 만들어둔 recap 3건·outlook 3건이 함께 버려졌다(리플레이 확인: split 렌더 1054자·`<li>` 6개 vs keynum 렌더 298자·0개).

**같은 날 함께 일어난 일 (이쪽은 정상 동작)**: `fetch_news`가 `그라운딩 출처 0건, 신호 ['no_grounding_sources']`로 뉴스 요약을 통째로 폐기했고(§31), 외국계 IB 코멘트도 지수 레벨 이상 후보 1건을 걸러 0건 저장했다. 둘 다 §0·§27 원칙대로 빈 섹션으로 발행됐다. **하지만 수치 카드 소멸은 이 안전장치와 무관하다** — 로그에 `num_cards`는 한 번도 등장하지 않고, 어떤 게이트도 그것을 지우지 않았다. 안전장치가 작동한 날과 결함이 겹쳐 원인이 가려지기 쉬웠다.

**감사 결과**: 최근 14영업일 스냅샷을 전수 검사해 "찍힌 형식의 본문 필드가 실제로 있는지" 확인한 결과 실제 사고는 08-13 하루뿐이었다(07-27·07-30 scenario는 `sc_left_items`/`sc_right_items`가 정상적으로 있었고, 08-06 `split`은 형식별 필드가 없는 형식이라 정상).

**수정** ([validate_analysis.py](../scripts/validate_analysis.py)):
- `FORMAT_BODY_FIELDS` — 형식별 '본문이 담기는 필드' 표. `generate_html.build_*_context()`가 실제로 읽는 키와 1:1로 맞춘다.
- `fix_format_payload_mismatch()` — 찍힌 형식의 본문이 전부 비었으면 `split`으로 내려앉히고 교정 로그를 남긴다. **내용은 온전한데 포장만 어긋난 것이므로 발행을 막지 않는다** — §0은 "짧거나 섹션이 비는 것은 감수하되 틀린 것은 감수하지 않는다"이고, 여기서 틀린 것은 없다. 오히려 내려앉히면 버려질 뻔한 recap/outlook 본문이 살아난다.
- 다만 **내려앉힐 내용조차 없으면**(`todays_view.recap`·`outlook`·`key_drivers` 전부 없음) 빈 브리핑이 나가므로 그때는 `blocks`에 넣어 발행을 차단한다.
- 미등록 형식은 판단 근거가 없으므로 조용히 통과시키지 않고 경고를 남긴다.
- 검증 본체에서 **다른 교정보다 먼저** 호출한다 — 이후 단계가 실제로 렌더될 필드를 대상으로 검사해야 한다.
- 테스트 `scripts/test_format_payload_mismatch.py`(10) — 실사고 리플레이 + 형식별 검출 + 차단 조건 + **로테이션 풀 전 형식이 표에 등록됐는지 확인하는 회귀 가드** 포함.

- **방지 룰(강제로 찍는 값은 그 전제가 성립하는지 함께 확인한다)**: LLM 응답에 코드가 필드를 덮어쓸 때는 "모델이 실제로 그렇게 답했는가"를 반드시 함께 검사한다. 덮어쓰기는 **불일치를 고치는 게 아니라 감춘다** — 형식 이름과 본문이 어긋난 채로 굳어져, 잡은 success·페이지는 200·아무도 모르는 상태가 된다(§20·§32와 같은 무음 실패 계열).
- **방지 룰(빈 컨테이너를 렌더하지 않는다)**: "데이터가 없으면 비운다"는 **섹션을 생략한다**는 뜻이지 **빈 껍데기를 그린다**는 뜻이 아니다. 반복 렌더 블록(`{% for %}`)을 템플릿에 넣을 때는 컬렉션이 비었을 때 무엇이 그려지는지 확인하고, 바깥 컨테이너까지 `{% if %}`로 감싼다.
- **방지 룰(한 값이 두 곳의 렌더를 동시에 가른다면 그 값은 검증 대상이다)**: `analysis_format`은 형식별 템플릿 선택과 `todays_view`의 2단 렌더 여부를 **동시에** 결정한다. 이런 값이 틀리면 피해가 한 섹션에 그치지 않는다. 분기 키로 쓰이는 필드는 값의 유효성뿐 아니라 **그 값이 요구하는 데이터가 있는지**까지 게이트한다.
- **재발 시 진단 순서**: ① 발행된 HTML에서 빈 컨테이너(`grid`·`wrap` 클래스 안이 공백)를 grep — 있으면 형식↔본문 불일치를 의심한다. ② `web/briefings/{date}/kospi/analysis_snapshot.json`의 `analysis_format`과 그 형식이 요구하는 필드가 실제로 있는지 대조한다. ③ 잡 로그에서 그 필드명을 grep — **한 번도 안 나오면 게이트가 지운 게 아니라 모델이 애초에 안 만든 것**이다(게이트가 지웠다면 반드시 교정 로그가 남는다). ④ 같은 날 다른 안전장치(뉴스 폐기 등)가 작동했더라도 그것과 혼동하지 말 것 — 로그에서 각각의 원인을 따로 확인한다.

### 35. 지난 장 복기 문장이 단어 단위로 쪼개져 보임 — flex li의 직접 자식이 여러 개일 때 (2026-08-18 실사고, 수정 완료)

**증상**: 2026-08-18 07:27 KST 코스피 예측 브리핑의 "지난 장 복기"(recap) 목록이 단어 단위로 쪼개져 세로로 나열됐다. "메모리 반도체 ETF(DRAM)가"가 "메모리" / "반도체" / "ETF(DRAM)" / "가"로 각각 한 줄씩 차지하며 좁은 칸에서 줄바꿈됐고, 그 사이사이 굵은 수치(`+6.49%`)도 별도의 좁은 칸으로 떨어져 나왔다. 사용자가 "페이지가 깨져 보인다"고 보고했다. 같은 카드의 "오늘 볼 것"(outlook) 목록은 정상이었다.

**근본 원인**: `.tv-col li{display:flex;gap:8px}`(불릿 `::before` + 텍스트 한 덩어리를 나란히 놓기 위한 규칙)가 recap `<li>`에는 텍스트와 `<b>{{...}}</b>` 태그가 **직접 자식**으로 들어 있는 상태에서 적용됐다.

```html
<!-- recap (버그) -->
<li>메모리 반도체 ETF(DRAM)가 <b>+6.49%</b>, 한국 증시 추종 ETF(EWY)가 <b>+3.94%</b> 폭등하며...</li>

<!-- outlook (정상) -->
<li><span><span class="tv-tag ...">관전</span>휴장 전날 외국인이 <b>3,038억원</b> 순매수했던...</span></li>
```

CSS flex 컨테이너의 자식은 (생성된 `::before` 포함) 각각 별도 flex item으로 취급되고, **인라인 요소(`<b>`)가 flex 컨테이너의 직접 자식이면 강제로 블록 레벨 flex item으로 승격**되며, 그 사이의 텍스트 조각(text node run)도 각각 **익명 flex item**이 된다. 즉 위 recap `<li>`는 `::before`(불릿) + 텍스트1 + `<b>`(+6.49%) + 텍스트2 + `<b>`(+3.94%) + 텍스트3, 총 6개의 flex item으로 쪼개졌다. `flex-shrink` 기본값이 1이라 컨테이너 폭(약 400px)에 맞춰 6개 item이 균등하게 짜부라지고, 각 item의 텍스트가 그 좁은 폭 안에서 다시 줄바꿈되면서 "단어가 세로로 나열된 것처럼" 보였다.

outlook `<li>`는 처음부터 전체 내용을 `<span>` 하나로 감싸고 있어(`<li><span>...</span></li>`) flex item이 **`::before` + `<span>` 단 2개**뿐이었다. `<span>` 내부의 `<b>`는 flex 컨테이너의 직접 자식이 아니므로 정상적인 인라인 흐름을 따라 문장이 그대로 렌더됐다 — 같은 클래스(`.tv-col li`)를 쓰면서도 마크업 구조 차이 하나로 한쪽만 깨진 것이다.

**수정** ([todays_view.html](../scripts/templates/sections/todays_view.html)): recap `<li>`도 outlook과 동일하게 내용 전체를 `<span>`으로 감쌌다.

```html
<li><span>{{ r.text | safe }}</span></li>
```

CSS는 건드리지 않았다 — `.tv-col li{display:flex}` 규칙 자체는 불릿·텍스트 2분할 레이아웃 의도대로 정상이고, 문제는 그 레이아웃이 기대하는 "직접 자식은 1개"라는 전제를 recap 템플릿만 어긴 것이었다.

**동일 패턴 감사**: `grep`으로 `<li>{{ ... }}</li>`(직접 자식, span 미포함) 형태의 템플릿을 전수 확인했다. `sector_focus.html`·`us_linked_story.html`도 같은 형태였지만, 그 목록이 속한 `.reason-block ul`은 **`<ul>` 자체가 `display:flex;flex-direction:column`**이라 `<li>` 하나하나가 이미 온전한 flex item이고(내부의 `<b>`는 flex 컨테이너의 직접 자식이 아니므로 무관), 문제가 재현되지 않는다. `li{display:flex}`(스타일리스트 자신이 flex 컨테이너인 경우)는 스타일시트 전체에서 `.tv-col li` 하나뿐이었다.

- **방지 룰(flex 컨테이너에 인라인 마크업을 직접 넣지 않는다)**: `display:flex`를 리스트 아이템(`li`) 자체에 걸어 "불릿 + 텍스트"를 배치하는 패턴을 쓸 때는, 그 li의 자식이 **항상 정확히 텍스트 파트 1개**여야 한다. 텍스트 안에 `<b>`·`<a>` 등 인라인 강조 태그가 들어갈 여지가 있다면(이 프로젝트는 수치 강조에 `<b>`를 광범위하게 쓴다) 반드시 `<span>`으로 감싸 li의 직접 자식을 1개로 고정한다. 이 프로젝트에서 `<b>` 태그가 없는 문장으로 먼저 만들어지고 나중에 수치 강조가 추가되는 템플릿이 있다면(recap이 그랬을 가능성이 높다), CSS가 처음엔 우연히 동작하다가 내용에 `<b>`가 섞이는 순간 조용히 깨진다.
- **방지 룰(같은 CSS 클래스를 쓰는 형제 요소를 서로 대조한다)**: `.tv-col li`처럼 여러 곳에서 재사용되는 클래스가 있고 그중 일부만 깨졌다면, 정상 동작하는 다른 인스턴스(이번엔 outlook)의 마크업과 **구조를 직접 대조**하는 것이 가장 빠른 진단이다. 클래스가 같으므로 CSS는 범인이 아니고, HTML 구조 차이를 찾으면 된다.
- **재발 시 진단 순서**: ① 문제 요소의 `getComputedStyle().display`를 확인해 `flex`/`grid` 등 자식을 개별 아이템으로 다루는 레이아웃인지 본다. ② 그렇다면 그 요소의 **직접 자식 개수**를 세어본다 — 인라인 태그(`<b>`·`<a>`)나 텍스트 노드가 여러 개 섞여 있으면 그 수만큼 flex item으로 쪼개진다. ③ 같은 클래스를 쓰는 다른 인스턴스가 정상이라면 그 마크업과 자식 구조를 대조한다. ④ 고칠 때는 CSS를 바꾸기 전에 "내용 전체를 감싸는 래퍼 하나만 자식으로 두면 되는가"를 먼저 검토한다 — 레이아웃 의도(불릿+텍스트 2분할)를 지키면서 구조만 맞추는 것이 가장 안전한 수정이다.

### 36. 종목 상세 페이지를 3+10종목으로 축소 — 랭킹·섹터 유니버스는 46종목 유지 (2026-08-19 결정)

**요청**: 종목 상세 페이지(`/stocks/{code}/`)를 삼성전자(005930)·현대차(005380)·SK하이닉스(000660) + "밤사이 미국 반도체 시황" 박스 종목(미국 10개 페이지 전부, `web/stocks/us/*`)만 남기고 나머지 한국 종목 43개는 제거. 단 종목 시그널 허브(`/stocks/`)의 랭킹·섹터 브라우저·특이신호는 계속 46종목 전체를 스캔·표시하되, 그중 상세 페이지가 없는 종목은 **클릭 불가(존재-게이트)**로만 처리 — 데이터 유니버스 자체는 줄이지 않는다.

**핵심 발견 — `stocks.json`이 다목적 파일**: 처음엔 `scripts/config/stocks.json`(46개)을 3개로 트리밍하려 했으나, 이 파일이 `scripts/fetch_movers_why.py`("코스피 주도주" 뉴스 근거 수집)의 종목 유니버스로도 쓰이고 있어 그대로 줄이면 그 기능까지 조용히 축소되는 부작용이 있었다. `stock_universe.json`(섹터·랭킹용, 46개)과 `stocks.json`(상세 페이지 생성 대상, 이번에 3개로)은 겉보기엔 같은 46개 코드 집합이지만 **서로 다른 파일이고 서로 다른 소비자**를 갖는다.

**해법 — 항목을 지우지 않고 `detail_page` 플래그로 분리**: `stocks.json`의 46개 항목은 그대로 두고, 각 항목에 `"detail_page": true/false`만 추가했다. `fetch_movers_why.py`는 이 필드를 무시하고 계속 46개 전체를 스캔한다. `generate_html.py`의 `build_all_stocks()`·`build_sector_pages()`, `verify_publish_gate.py`의 `expected_pages()`는 `detail_page: true`인 항목만 대상으로 삼도록 필터를 추가했다.

**기존 존재-게이트 패턴을 확장**: `detail.html`의 KR 피어 섹션(`p.linked`)과 `_pick_detail_url()`(브리핑 종목 픽)은 이미 "파일이 실제로 있을 때만 링크"하는 패턴이 구현돼 있었다. 이번에 같은 패턴을 두 곳에 새로 적용했다.
- `stock_sector.html`/`build_sector_pages()` — 섹터 카드에 `linked` 플래그 추가, `{% if s.linked %}<a>{% else %}<div>{% endif %}`
- `web/assets/main.js`의 브리핑 사이드바 "오늘의 특이 신호" 위젯 — `/api/signals`가 46종목 전체를 스캔해 무조건 `<a href>`로 링크하던 것을, `sidebarSignalRowHtml()`을 최상위로 끌어올려 `DETAIL_PAGE_CODES`(keep-list) 존재-게이트를 적용. 테스트 가능하도록 `window.__dsTestables`에 노출.
- `web/assets/stocks-home.js`의 `STOCK_PAGES` — 기존엔 스냅샷 로드 후 `_ks.forEach(c => STOCK_PAGES[c]=1)`로 46종목 전부를 "페이지 있음"으로 잘못 넓히고 있었다(예전엔 stocks.json=stock_universe.json이라 안전했지만 이번에 갈라지며 위험해짐). 이 broadening을 제거해 초기값(keep-list 3개)이 그대로 유지되게 했다. 토스트 문구도 "준비 중"(일시적 뉘앙스) → "상세 페이지를 제공하지 않아요"(영구적 사실)로 수정.

**프론트 keep-list가 두 곳에 하드코딩됨**: `stocks-home.js`의 `STOCK_PAGES`와 `main.js`의 `DETAIL_PAGE_CODES`는 둘 다 `{'005930','000660','005380'}`을 직접 하드코딩한다. 서버가 `stocks.json`을 클라이언트에 노출하지 않으므로 불가피한 선택이었다 — **`stocks.json`의 `detail_page` 필드를 바꿀 때는 이 두 JS 상수도 반드시 함께 맞춰야 한다.** 어긋나면 존재하지 않는 페이지로 링크하거나(과다 링크), 실제로 있는 페이지를 링크 안 하는(과소 링크) 조용한 불일치가 생긴다.

**로컬 stale 데이터로 재생성하지 않고 surgical edit**: 작업 시점 로컬 `web/data/stocks-snapshot.json`이 5일 전(08-14) 데이터였다(§23·§28 계열 위험 — 로컬에서 `generate_html.py --stocks/--sectors`를 그대로 돌리면 라이브 페이지의 가격·등락률을 5일 지난 값으로 덮어쓸 뻔했다). 대신 `origin/main`에서 최신 3개 상세 페이지·8개 섹터 페이지 HTML을 가져와, 제거 대상 종목을 가리키는 `<a>` 태그만 정규식으로 `<div>`로 치환하는 방식(peer 3건, 섹터 카드 43건)으로 정정했다. 가격·스파크라인 등 실측 수치는 한 글자도 건드리지 않았다(diff로 확인). sitemap.xml만 `generate_html.py --write-list-only`(가격 데이터를 읽지 않는 유일한 플래그)로 안전하게 재생성해 3(KR)+10(US)+8(섹터)로 정리했다.

**Fast-forward 충돌**: 작업 중 로컬 브랜치가 origin보다 29커밋 뒤처져 있어(자동 발행 커밋들), stash 후 pull하면서 8개 섹터 페이지에 병합 충돌이 났다. 자동 3-way 병합을 신뢰하지 않고, `origin/main`에서 다시 최신 파일을 받아 unlink 스크립트를 처음부터 재실행하는 방식으로 정리했다(이 프로젝트에서 반복적으로 강조되는 "실측 데이터가 걸린 병합은 자동 병합 결과를 신뢰하지 말고 소스에서 재현할 것" 원칙과 동일).

**의도적으로 받아들인 남은 갭**:
- **이미 발행된 과거 브리핑의 종목 픽 링크는 소급 수정하지 않는다.** `_pick_detail_url()`은 생성 시점에 파일 존재를 확인하므로, 이번에 제거한 43종목이 과거 어느 브리핑에서 픽으로 뽑혔을 때는 그 시점엔 상세 페이지가 있어 링크가 걸렸다. 지금 그 페이지를 지웠으니 그 과거 브리핑들의 링크는 죽은 링크가 된다. §23·§28 선례대로 발행된 브리핑은 재생성하지 않는다 — 새로 발행되는 브리핑부터는 `_pick_detail_url()`이 자동으로 안전하게 링크를 생략한다.
- **`#etf-detail`류의 완전 고립된 죽은 화면과는 다른 층위**: 이번 43종목은 "링크는 없지만 랭킹·섹터·특이신호에는 계속 나타나는" 상태다 — 완전히 안 보이는 게 아니라 "보이지만 못 들어가는" 상태로 남는다. 이게 사용자가 명시적으로 선택한 방향(Q1 답변)이다.

- **방지 룰(설정 파일의 다목적성을 먼저 확인한다)**: 설정 파일 하나를 줄이거나 지우기 전에 그 파일을 읽는 **모든** 소비자를 grep으로 확인한다. 이번에 `stocks.json`이 상세 페이지 생성과 뉴스 근거 수집 두 기능에 동시에 쓰이는 걸 뒤늦게 발견했다 — 처음 계획대로 그냥 항목을 지웠다면 `fetch_movers_why.py`의 스캔 범위가 조용히 줄어드는 부작용이 났을 것이다.
- **방지 룰(존재-게이트가 있는 곳과 없는 곳을 전수 확인한다)**: "생성 대상을 줄인다"는 변경은 반드시 "그 대상으로 링크하는 모든 지점"의 존재-게이트 여부를 먼저 감사해야 한다. 이미 게이트가 있는 곳(피어 섹션, 종목 픽)은 안전하지만, 없는 곳(섹터 카드, 사이드바 위젯, 프론트 `STOCK_PAGES`의 과다 확장)은 페이지 삭제와 동시에 즉시 죽은 링크를 만든다.
- **재발 시 진단 순서**: ① 어느 종목의 상세 페이지가 있어야 하는지는 `scripts/config/stocks.json`의 `detail_page` 필드가 정본이다. ② 프론트에서 링크가 안 걸리는데 있어야 할 종목이면 `stocks-home.js`의 `STOCK_PAGES`·`main.js`의 `DETAIL_PAGE_CODES`가 `stocks.json`과 동기화됐는지 확인. ③ 섹터 페이지·상세 페이지 카드가 죽은 링크를 걸고 있으면 `build_sector_pages()`/`build_all_stocks()`의 `linked` 계산이 최신 `detail_page` 값을 반영해 재생성됐는지 확인(다음 마감 배치 전이면 아직 반영 전일 수 있다).

### 37. 코스피 아침 브리핑 2회 미발행 — 최상급 주장 차단 + 차단 알림 자체가 400으로 실패 (2026-08-26 실사고, 수정 완료)

**증상**: 2026-08-26 07:25 KST 코스피 예측 브리핑이 발행되지 않았다. 잡은 "실패"로 끝났지만
아무도 몰랐고, 사용자가 오전에 직접 발견했다. 2026-08-24에도 같은 이유로 차단됐는데 그때는
재실행으로 넘어가 사고가 드러나지 않았다.

**차단 사유(게이트는 정상 동작)**: 분석 본문이 "S&P500이 사상 최고 수준을 다시 확인했고"라고
썼는데 실측은 7,677로 52주 고점 7,799 대비 **-1.6%**였다. §28 `validate_index_superlatives()`가
잡아 스칼라 산문이므로 발행을 차단했다 — 설계대로다. 문제는 왜 모델이 그 주장을 했는지,
그리고 왜 차단 사실이 아무에게도 전달되지 않았는지다.

**근본 원인 세 겹**:

1. **모델에게 판단 근거가 없었다.** 프롬프트에 들어가는 지수 데이터는 가격과 등락률뿐이라,
   "+0.32% 상승"에서 "사상 최고"로 넘어가는 비약을 막을 실측이 하나도 없었다. `get_ticker_full()`이
   이미 1년치 일봉을 읽고 있어 52주 고·저는 **추가 비용 0**으로 낼 수 있었는데 쓰지 않고 있었다.
2. **차단 알림이 자기 서식 때문에 죽었다.** `send_admin_alert()`가 `parse_mode=HTML`로 보내는데,
   차단 사유 문자열에는 **모델이 쓴 산문이 그대로** 들어간다. 그 산문에 `<b>` 태그가 섞이고
   길이 제한으로 잘리면 텔레그램이 `HTTP 400 Bad Request`로 거절한다. 로그에
   `[validate] 관리자 알림 실패: HTTP Error 400` 한 줄만 남고 끝났다 — **가장 중요한 알림이
   가장 확실하게 실패하는 구조**였다(차단 사유에 산문이 들어갈수록 400 확률이 올라간다).
3. **뉴스가 매일 0건이라 모델이 시장 데이터만으로 서사를 지어낸다.** `fetch_news`가
   `그라운딩 출처 0건 → 뉴스 요약 전체 폐기`를 **최근 조사한 8영업일 전부**에서 기록했다
   (§31이 지목한 무출처 수집 구조가 지금 100% 실패 중이다). 뉴스가 없으면 모델은 지수 등락률
   같은 얇은 근거 위에 관점을 세우게 되고, 그만큼 최상급 같은 과장으로 새기 쉽다.

**수정**:
- `fetch_data.get_ticker_full()` — `high_52w`·`low_52w`·`pct_from_52w_high/low` 추가(1년치를
  이미 읽으므로 추가 조회 없음). 코스피 페이로드에 `index_52w`(S&P500·나스닥·다우·SOX) 노출.
- `call_claude._index_position_directive()` — 각 지수가 52주 고·저 대비 어디인지와
  "최상급은 52주 고점의 99% 이상일 때만"을 프롬프트에 명시(§24·§29의 데이터 기반 지시 주입 패턴).
  **임계 1%는 게이트의 `high_52w * 0.99`와 같은 값으로 고정**했다 — 어긋나면 프롬프트가 허용한
  표현을 게이트가 막는 모순이 생긴다. 테스트가 두 값을 함께 잡는다.
- `send_admin_alert()` — HTML 실패 시 **평문으로 한 번 더** 보내고, 실패 시 텔레그램이 준
  사유 본문을 로그에 남긴다. 호출부는 차단 사유를 `html.escape`한다.
- 테스트: `test_index_position_directive.py`(7), `test_admin_alert_fallback.py`(5).

**장 시작 후 재생성 — `DS_PIN_SESSION_DATE`**: 이번 정정은 09:35 KST(개장 후)에 돌렸는데,
그대로 재실행하면 `fetch_data`·`validate_analysis`가 **당일 진행 중인 미완성 일봉**을 최신값으로
잡는다(실측 확인: 오늘 캔들 거래량이 전일의 1/6). 07:25 발행 시점엔 한국·아시아 시장이 닫혀
있으므로 그 전제를 재현하는 pin을 넣었다. `daily_report.yml`의 `pin_session_date` 입력 →
`DS_PIN_SESSION_DATE` → 그 날짜 이후 한국·아시아 데이터를 아직 없는 것으로 취급한다.
**미국·선물·환율은 07:25에도 이미 열려 있었으므로 제외한다** — 전부 일괄로 되돌리면 오히려
07:25 상태와 멀어진다. 정정 결과 픽 4종목 가격이 전부 08-25 종가와 정확히 일치함을 확인했다.

- **방지 룰(게이트가 막는 표현은 프롬프트에서 먼저 막는다)**: 검증 게이트를 추가할 때는 그
  게이트가 판정에 쓰는 **실측을 모델에게도 준다**. 근거 없이 "쓰지 마라"만 지시하면 모델은
  자기가 아는 얕은 신호로 판단해 같은 실수를 반복한다. 게이트 임계와 프롬프트 임계는 반드시
  같은 값이어야 하고, 두 값을 함께 고정하는 테스트를 둔다.
- **방지 룰(알림은 자기 서식 때문에 죽으면 안 된다)**: 실패를 알리는 경로에 **모델·사용자가
  만든 문자열을 서식 있는 채로 실어 보내지 않는다.** 이스케이프하고, 그래도 실패하면 서식을
  포기하고 평문으로 보낸다. 알림 실패는 조용하고, 조용한 실패는 사고를 하루 늦춘다(§32).
- **방지 룰(발행 차단은 "성공적으로 막았다"가 아니라 "오늘 브리핑이 없다"이다)**: 게이트가
  정상 작동해 차단한 날은 **구독자에게 아무것도 나가지 않은 날**이다. 차단 자체는 옳지만
  거기서 끝내면 안 된다 — 왜 모델이 그 주장을 했는지를 그날 안에 고치지 않으면 다음 날 똑같이
  막힌다(08-24 → 08-26 이틀 만에 재발했다).
- **알려진 미해결**: (a) `fetch_news` 그라운딩 출처가 매일 0건이라 코스피 아침 브리핑이 사실상
  뉴스 없이 나가고 있다 — §31이 처방한 "문서를 먼저 가져오고 LLM은 고르고 요약만" 구조 전환이
  필요하다. (b) ~~같은 종목·같은 날짜의 일봉 종가가 **토스와 네이버에서 최대 4% 어긋난다**~~ → **§48에서 원인 확정·해결(토스 일봉이 공식 종가가 아님)**
  (삼성전자 08-21 토스 270,000 vs 네이버 281,500). 실측 1순위와 폴백이 서로 다른 값을 주는
  것은 §0 정면 위반이므로 별도 조사가 필요하다. (c) `_closes_from_toss_candles()`가 거래량을
  `tradingVolume` 키로 찾는데 토스 실제 응답은 `volume`이라, `vol_mult`가 항상 비어 있다.

### 38. 뉴스 수집이 매일 0건이던 진짜 이유 — JSON 스키마 압박이 검색 자체를 막았다 (2026-08-26, 수정 완료)

**증상**: `fetch_news`가 조사한 8영업일 **전부** `그라운딩 출처 0건 → 뉴스 요약 전체 폐기`였다.
§31이 "출처가 몇 건 붙었는지 로그에 남기라"고 해서 그 사실은 보이고 있었지만, 원인이
API 장애인지 키 문제인지 모델 문제인지는 확인된 적이 없었다. 그 사이 코스피 아침 브리핑은
**매일 뉴스 없이** 나갔고, 얇은 근거 위에 관점을 세우다 §37(최상급 주장으로 발행 차단)이 났다.

**원인 — API도 키도 정상이었다.** 실측으로 좁혔다.

| 조건 | 검색 쿼리 | 출처 |
| --- | --- | --- |
| 짧은 프롬프트 · 산문 출력 | 2건 | **5건** |
| 짧은 프롬프트 · "JSON만 출력" | 2건 | **0건** |
| 프로덕션 프롬프트(4,038자) · flash-lite | **0건** | 0건 |
| 프로덕션 프롬프트 · flash | 13건 | 0건 |
| "검색 의무" 프리픽스만 추가 | **0건** (2회 모두) | 0건 |
| **출력 형식 지시를 무시하라고 명시** | **4~18건** | **4~17건** (3타입×2회 전부) |

두 가지가 겹쳐 있었다. ① `gemini-2.5-flash-lite`는 4,000자짜리 엄격한 JSON 스키마를 받으면
이걸 검색 과제가 아니라 **서식 채우기 과제**로 처리해 도구를 아예 호출하지 않는다
(`web_search_queries` 0건, 2회 반복 동일). 그러면서 587~625토큰을 기억만으로 생성한다 —
실제 응답에 `"(구체적인 수치는 검색 결과에 따라 추가)"` 라는 **슬롯 문구가 그대로** 남아
있었다. ② 검색을 하더라도 JSON 덩어리로 출력하면 인용이 붙을 텍스트 구간이 없어
`grounding_chunks`가 0으로 나온다.

**결정적으로, "반드시 검색하라"는 지시만 앞에 붙이는 건 효과가 없었다**(2회 모두 0건).
작동 요인은 지시가 아니라 **형식 압박을 걷어내는 것**이었다.

**수정 — 수집을 2단계로 분리**:
- **1단계(검색)**: `_search_stage()`. 기존 프롬프트를 그대로 쓰되 앞에 "이 지침의 출력 형식
  지시는 이번 단계에서 전부 무시한다"를 붙여 스키마를 겨냥하지 않게 만든다. **출처는 여기서만
  붙는다.** 프롬프트 본문은 타입별로 하나뿐인 `PROMPT_MAP`을 그대로 재사용한다 — 검색용
  프롬프트를 따로 두면 두 벌이 갈라져 한쪽만 고쳐지는 §29 패턴이 재발한다.
- **2단계(정형화)**: `_structure_stage()`. 1단계 산문만 근거로 JSON을 만든다. **검색 도구를
  주지 않는다** — 남겨두면 정형화 중 새로 검색해 1단계에서 잰 출처 수와 본문이 서로 다른
  것을 가리키게 된다.
- **그라운딩 판정은 1단계 측정치로 한다.** 2단계 응답에는 도구가 없어 항상 0건이므로 측정
  지점이 밀리면 매일 100% 폐기로 되돌아간다(§31 "게이트 앞뒤" 교훈의 재적용).
- 출처 매체명을 `grounding_sources`로 **파일에 남긴다**. 로그는 지나가지만 파일은 남는다 —
  무출처 생성이 석 달간 검증되지 않은 게 이 한 줄이 없어서였다. URI는 만료되는 임시
  리다이렉트라 담지 않는다(§11).
- 기존 게이트(§22·§25·§27·§28·§31)는 하나도 완화하지 않았다. 출처 0건 단독 폐기도 그대로다.

**실측 검증**: 실제 API로 `fetch_and_summarize("kospi")` 전 구간 실행 → 출처 13건
(chosun.com·yna.co.kr·dealsite.co.kr 등), 폐기 신호 없음, 뉴스 정상 통과. 그런데도 본문에
"나스닥 0.77% 하락"이 섞였고(실측 +0.66%), §27 실측 대조 게이트가 **그 두 항목만 정확히
제거**하고 나머지는 통과시키는 것까지 확인했다. 테스트 12건(`test_two_stage_news.py`).

- **방지 룰(도구를 쓰게 하려면 형식을 비워둔다)**: LLM에게 "검색해서 정해진 스키마로 내라"를
  한 번에 시키면 작은 모델은 스키마를 먼저 만족시키고 도구를 건너뛴다. **도구 사용과 형식
  준수는 다른 호출로 나눈다.** "반드시 도구를 써라"는 지시로는 못 고친다 — 실측으로 확인됐다.
- **방지 룰(신호가 0이면 원인을 좁히기 전엔 아무것도 모른다)**: `no_grounding_sources`는
  "검색을 안 했다"와 "검색했지만 출처가 안 붙었다"를 **구분하지 못한다.** 두 원인은 처방이
  정반대다(전자는 프롬프트, 후자는 출력 형식). 게이트 신호를 만들 때는 그 신호가 **어떤
  원인들을 뭉뚱그리는지**를 함께 적고, 뭉뚱그린 채로 오래 두지 않는다.
- **방지 룰(API가 도는 것과 그라운딩이 되는 것은 다르다)**: 응답이 200으로 오고 JSON이 파싱돼도
  그 내용이 검색에서 왔다는 보장은 없다. `web_search_queries`와 `grounding_chunks`를 **둘 다**
  본다 — 전자가 0이면 검색 자체를 안 한 것이고, 전자만 있고 후자가 0이면 근거가 안 붙은 것이다.
- **남은 목표**: 이건 §31이 처방한 **RSS 우선 수집**(문서를 먼저 가져오고 LLM은 고르고 요약만)
  으로 가는 징검다리다. 같은 저장소의 `fetch_news_live.py`·`fetch_ib_korea_views.py`가 이미
  그 방식이고 둘 다 실제 원문 URL을 갖는다. 2단계 분리는 출처를 되살렸을 뿐, 날짜·수치가
  기사에서 그대로 오는 구조는 아니다 — 여전히 §27 게이트가 틀린 수치를 걸러내야 한다.

### 39. 브리핑 사이드바 '시장 지표' — VIX + 공포·탐욕 지수 2개 (2026-08-31 결정)

코스피·미국 브리핑 사이드바의 `시장 지표` 패널을 **VIX**와 **공포·탐욕 지수(CNN Fear &
Greed Index)** 두 항목만 남기도록 축소했다. 나스닥·필라델피아 반도체·나스닥100 선물 행은
화면에서 제거 — 이 패널의 역할을 '지수 시세'가 아니라 '투자 심리'로 고정한 것이다.

**수집은 줄이지 않았다.** `market_data_js`의 `nasdaq`·`sox`·`nq`는 그대로 수집한다 —
`call_claude` 프롬프트와 '지금 코스피' 밴드 등이 계속 소비하기 때문이다. **화면에서 뺀다고
수집까지 같이 줄이지 말 것**(§36에서 `stocks.json`을 줄이려다 `fetch_movers_why`의 스캔
범위가 조용히 축소될 뻔한 것과 같은 함정 — 설정·데이터의 소비자를 먼저 전수 확인한다).

**공포·탐욕 지수 데이터 소스 — `production.dataviz.cnn.io`가 정본이다.** 2026-06-03에
제거된 옛 구현은 `api.alternative.me/fng/`를 호출하면서 docstring엔 "CNN Fear & Greed
Index"라고 적어놨다. **그건 암호화폐 심리 지수**라 라벨과 실데이터가 다른 §0 위반 상태였고,
그래서 지표 자체가 통째로 제거됐다. 지금 표시하는 정의("투자자 심리를 0~100으로 계량화한
시장 심리 지표")는 CNN 지수의 정의다 — **alternative.me로 되돌아가지 말 것.**

**러너 수집 가능 여부는 실측으로 확인했다** (일회성 워크플로우, 확인 후 삭제):

| 조건 | 결과 |
| --- | --- |
| 러너 위치 | Chicago · AS8075 Microsoft(Azure) — 미국 데이터센터 IP |
| 브라우저 헤더 있음 | **HTTP 200**, 로컬(한국 IP)과 동일한 값 |
| 헤더 없음 (대조군) | **HTTP 418** |

→ `get_fear_greed()`의 `User-Agent`·`Accept`·`Referer` **헤더 3줄은 필수다.** 지우면 418로
조용히 실패하고 행만 사라진다. 로컬에서 되는 것과 러너에서 되는 것은 다른 문제이므로,
외부 API를 새로 붙일 때는 **러너에서 한 번 직접 호출해보고** 붙인다.

**표시 규칙 — 공포·탐욕은 반원 다이얼, VIX는 그 아래 한 줄.**
프로토타입 4안(행+뱃지 / 구간 게이지 / 2열 카드 / 다이얼)을 실측값으로 그려 비교한 뒤
다이얼안으로 결정했다([2026-08-31-sidebar-sentiment-variants.html](prototypes/2026-08-31-sidebar-sentiment-variants.html)).
두 지표의 위계가 갈리고 세로 공간을 더 쓰는 것은 알고 받아들인 트레이드오프다 — 심리 지표는
"지금 어느 구간인가"가 핵심인데 숫자만으로는 그게 전달되지 않는다.

- 다이얼 아치는 반지름 82 · 중심 (100,100) **고정 좌표**라 템플릿에 상수로 둔다. 구간 경계는
  25/45/55/75점. `build_fng_dial()`은 **바늘 각도만** 계산한다 — `(점수−50)×1.8`
  (0점=−90°, 50점=0°, 100점=+90°). 좌표를 손대려면 이 각도 공식과 함께 고쳐야 한다.
- **양 끝이 모두 빨강인 것은 의도**다 — 공포뿐 아니라 탐욕(과열)도 위험 구간이다.
  안쪽=노랑, 중립=초록. 바늘·점수는 `var(--ink)`라 다크 모드에서 자동으로 뒤집힌다.
- 전일 대비는 %가 아니라 **포인트 차이**(`전일 58.2 · -3.7p`). 0~100 점수라 등락률은 의미가
  없다. `prev_close`가 없으면 `0.0p`로 채우지 않고 줄 자체를 비운다(보합이라는 틀린 주장 방지).
  이 줄은 빨강·파랑으로 칠하지 않는다 — 점수 상승을 "시장이 올랐다"로 오독하게 된다.
- **표시상 0.0p가 되는 구간(`|차이| < 0.05`)도 같은 이유로 비운다** — 2026-08-31 첫 발행에서
  확인된 구조적 결함이다. CNN은 미국 정규장이 열리기 **전에** 직전 종가를 `previous_close`로
  넘기는데 새 세션 값은 아직 없어 `score`와 `previous_close`가 **완전히 같아진다**. 미국
  브리핑은 21:15 KST(= 프리마켓)에 나가므로 이 구간에 항상 걸려, 비우지 않으면 **매일**
  `+0.0p`가 찍힌다. 소수 13자리가 우연히 일치할 일은 없으므로 이 조건은 사실상 "아직 갱신
  안 됨"만 잡는다. 코스피 아침(07:25 = 18:25 ET, 미국 마감 후)은 이 구간에 걸리지 않는다.
- **CNN `timestamp`는 데이터 시점이 아니라 조회 시점일 수 있다.** 롤오버 구간에서 값은 직전
  세션 것인데 `timestamp`는 현재 시각으로 찍힌다 — 그래서 `_fng_is_fresh()`의 신선도 게이트는
  "수집이 죽었나"는 잡아도 "값이 새 세션으로 갱신됐나"는 판별하지 못한다. 두 질문을 혼동하지 말 것.
- 라벨은 CNN이 준 `rating`을 우선 쓰고, 없거나 모르는 값일 때만 점수 구간으로 되돌린다 —
  경계값(45·55·75)에서 CNN 표기와 어긋나지 않게.
- VIX 행은 기존 `vix-badge` 팔레트를 그대로 쓴다(안정/보통/경계/불안/극단). 건드리지 않았다.
- `_fng_is_fresh()` 신선도 게이트 — `asof`가 5일보다 오래되면 표시하지 않고 경고를 남긴다
  (3일 연휴+주말 버퍼). 수집이 죽었을 때 낡은 점수가 계속 진짜처럼 보이는 것을 막는다(§20).
  게이트는 `build_fng_dial()` 안에 있다 — 다이얼 경로가 이 게이트를 우회하지 않는지 테스트가 지킨다.
- 두 지표가 **모두** 없으면 패널 자체가 사라진다(`kospi.html`·`us.html`의
  `{% if market_items or fng_dial %}`). 다이얼만·VIX만 있어도 정상 렌더된다.

**모델에는 여전히 주지 않는다.** `call_claude`의 [필수 규칙 5-1](공포탐욕지수 언급 금지)은
유지한다 — 이 지수는 사이드바 표시용으로만 수집되고 프롬프트에 들어가지 않으므로, 모델이
쓰는 어떤 수치도 실측이 아니다. 나중에 모델에 주기로 하면 그때 이 규칙을 함께 고쳐야 한다.

테스트: `scripts/test_market_items.py`(9) — VIX 행만 남는지, 다이얼 바늘 각도(0·50·100점 포함)·
라벨 폴백·신선도 게이트(다이얼 경로 포함)·전일값 결측 처리.


### 40. 크레딧 소진으로 마감 브리핑 미발행 — 실패가 아무에게도 도달하지 않았다 (2026-08-31 실사고)

**증상**: 16:25 KST `kospi-close-briefing` 잡이 실패해 그날 마감 브리핑이 발행되지 않았다.
사용자가 몇 시간 뒤 직접 발견했다.

**원인**: Anthropic API 크레딧 소진. 데이터 수집·뉴스·증권가 시황·목표주가까지 10개 스텝이
전부 성공하고 11번째 `✨ Claude 마감 시황 분석`에서 죽었다.

```
[call_claude] ERROR: Error code: 400 - invalid_request_error
'Your credit balance is too low to access the Anthropic API.'
```

같은 날 07:25 코스피 브리핑은 정상 발행됐으므로, 크레딧은 그 사이에 바닥났다.

**진짜 문제는 크레딧이 아니라 침묵이었다.** 잡이 `failure`로 끝났지만 GHA는 아무도 보고
있지 않고, 알림도 없었다. §32에서 `continue-on-error`가 실패를 성공으로 표시하는 문제를
고쳤는데, 이번은 **정직하게 failure로 끝났는데도 그 사실이 사람에게 도달하지 않은** 경우다.
결론은 같다 — 실패 표시와 실패 전달은 다른 문제다.

**수정** ([call_claude.py](../scripts/call_claude.py)): `alert_api_failure()` 신설.
Claude API 호출이 실패하면 `send_admin_alert()`로 관리자 텔레그램에 브리핑 종류·날짜·오류와
원인 힌트를 보낸다. 크레딧 소진/인증 실패/권한/레이트 리밋을 문자열로 구분해 대응 방법까지
적는다. **평문으로 보낸다** — 오류 문자열에 `<>`가 섞여도 서식 때문에 알림이 죽지 않게(§37).
알림 실패는 삼켜서 원래 오류를 가리지 않는다. 테스트 `test_call_claude_api_alert.py`(6).

`TELEGRAM_ADMIN_CHAT_ID`는 워크플로우 레벨 `env:`에 있어 3개 브리핑 잡 모두에 상속된다
(잡 레벨 `env:`는 동명 키만 덮어쓰고 나머지는 그대로 물려받는다).

- **방지 룰(발행을 멈추는 실패는 사람에게 도달해야 한다)**: 잡이 실패로 끝나는 것과 그 사실이
  누군가에게 전달되는 것은 다르다. **아무도 GHA 대시보드를 상시로 보지 않는다.** 파이프라인이
  통째로 멈추는 지점(외부 API 호출, 발송, 배포)에는 실패 알림을 붙인다.
- **방지 룰(외부 서비스 잔액·쿼터는 조용히 바닥난다)**: 크레딧·쿼터·구독 만료는 코드 결함이
  아니라서 테스트로 잡히지 않고, 바닥나는 순간까지 아무 신호가 없다. 오류 메시지 문자열로
  분기해 "무엇을 해야 하는지"까지 알림에 적어야 대응이 빨라진다.
- **재발 시 진단 순서**: ① `gh run list`로 실패한 잡을 찾고 `gh run view <id> --log-failed`로
  **실제 오류 줄**을 읽는다(추측 금지). ② 크레딧/인증 오류면 최소 요청 1건으로 현재 상태를
  직접 확인한다 — 지금도 막혀 있으면 이후 예약 실행이 전부 같은 이유로 실패한다.
  ③ 충전 후에는 별도 조치 없이 다음 예약 실행부터 정상화된다. 당일 유실분은
  `gh workflow run daily_report.yml -f briefing_type=<종류>`로 수동 재실행한다.


### 41. 종목 픽에 산출 근거를 노출한다 — 손절선은 실측 20일선으로 검증 (2026-09-02)

**배경**: 픽 카드가 진입가·목표가·손절가를 **구체적 원화 금액으로만** 보여주고 있었다.
근거 없는 구체적 가격은 금융 라이선스 없는 **매매 신호**로 읽힌다(애드센스 YMYL 심사에서
가장 걸리는 지점이고, 그건 콘텐츠 양으로 상쇄되지 않는다).

**조사에서 확인한 것**: `entry`·`target`·`stop`은 **LLM이 생성한 값**이다(`call_claude`
프롬프트가 "명확한 목표가와 손절가"를 요구한다). `validate_analysis`는 스케일 교정만 할 뿐
산식으로 계산하지 않는다. 그런데 `action_guide`는 이미 **"손절: 20일선(약 1,605,000원)
이탈 시"처럼 근거를 주장**하고 있었다. 2026-09-02 실측 대조 결과 그 주장은 정확했다
(SK하이닉스 오차 -0.17%, 삼성전자 -0.26%). **문제는 그게 우연히 맞은 것인지 아무도 확인하지
않았다는 점**이다 — §22·§24·§28에서 반복된 "산문 속 주장에 게이트가 없다" 패턴 그대로다.

**수정**:
- `_closes_to_realdata()`가 이격률뿐 아니라 **MA 절대 수준**(`ma20_level`·`ma200_level`)도
  낸다. 이미 계산하던 시리즈의 마지막 값이라 추가 조회 비용은 0이다.
- `_verify_stop_against_ma20()` 신설 — `action_guide`가 20일선을 근거로 **주장할 때만**
  실측 20일선과 대조하고, ±3%를 벗어나면 손절가를 실측값으로 교정한다. `stop_pct`도 현재가
  기준으로 다시 만들고, 현재가를 못 읽으면 **지운다**(틀린 비율을 남기지 않는다).
  20일선을 근거로 내세우지 않은 픽은 판단하지 않는다 — 다른 근거(전 저점 등)일 수 있다.
- 카드에 근거 한 줄 추가 — "손절선은 실측 20일 이동평균(1,607,750원)을 기준으로 잡았어요.
  **진입·목표는 AI가 제시한 참고 범위이고 매매 신호가 아니에요.**" + 면책 고지 링크.
  **어느 숫자가 실측이고 어느 것이 AI 판단인지 구분해 밝히는 것이 핵심이다.**
- `ma20_level`이 없으면 그 문장이 통째로 빠진다(§0 — 없으면 비운다). 기존 발행본은
  스냅샷에 이 필드가 없으므로 손절선 문장 없이 나머지 고지만 나온다 — 정상이다.
- 테스트: `scripts/test_pick_basis.py`(8) — 실사고 값 리플레이(오차 -0.17%는 손대지 않음)·
  오교정 방지·표기 변형(20일선/20 일 선/20MA/MA20)·fail-open·미국 달러 표기.

- **방지 룰(근거를 내걸면 그 근거를 검증한다)**: 화면에 "20일선 기준"이라고 쓰는 순간 그
  값은 **검증 대상**이 된다. 근거를 노출하는 것과 그 근거가 맞는지 확인하는 것은 한 쌍이다 —
  근거만 쓰고 게이트를 안 두면, 화면이 스스로 모순되는 상태(실측 20일선 옆에 다른 손절가)를
  만들 수 있다.
- **방지 룰(LLM 생성 수치와 실측을 화면에서 구분한다)**: §0은 "화면에 표시되는 모든 수치는
  실측"을 요구하지만, 예측·픽처럼 **판단**을 파는 영역에는 LLM 생성 수치가 남는다. 그것을
  없앨 수 없다면 **어느 것이 실측이고 어느 것이 판단인지 화면에서 밝힌다.** 섞어놓고 전부
  실측처럼 보이게 두는 것이 가장 나쁘다.
- **`pick.ma20`(미니차트용 시리즈)과 `pick.ma20_level`(손절 기준선 절대값)은 다른 값이다.**
  이름이 비슷하니 섞지 말 것 — 시리즈를 스칼라 자리에 넣으면 카드가 조용히 깨진다.


### 42. 섹터 페이지 본문 보강 + 홈 섹터 화면 제거 (2026-09-02)

**홈의 섹터 화면(`#sector`)을 제거했다.** 섹터 상세 페이지로 가는 링크가 **0개**였고(JS도
주입하지 않았다), '패시브 쏠림' 블록은 "실측 소스가 없어요" 자리표시자였다. 섹터 브라우징은
`/stocks/sector/{key}/` 정적 페이지 8종이 계속 담당한다.

- `stocks-home.js`에서 섹터 화면 모듈 150줄과 초기화 호출을 뺐다. **`SECTOR_LABELS`·`secLbl`은
  남긴다** — 섹터 박스(sbx) 위젯이 계속 쓴다(§36 소비자 전수 확인).
- `go('sector')` 폴백 2곳을 `go('home')`으로 바꿨다 — 없는 화면으로 보내면 안 된다.
- 서브탭 4개 → 3개. `sector-screen.test.mjs`(24) 삭제, `ds-subnav.test.mjs`를 3탭 기준으로 갱신.
  내부 화면 예시는 살아 있는 flow-map으로 교체했는데, **`id='flow'`인데 `screen='flow-map'`이라
  `go()`에 넘어가는 값은 `'flow-map'`이다**(섹터는 둘이 같았다).
- 고아 참조는 **eslint `no-undef`가 2건 잡아냈다**(`secShowSector`·`secActiveKey`). 그 검사를
  넣어둔 이유 그대로다 — 문법 검사·단위 테스트는 통과했다.

**섹터 페이지 본문을 980자 → 1,905~2,124자로 늘렸다.** 기존엔 본문의 거의 전부가 숫자 나열이고
산문은 두 문장(46자)뿐이었다.

- **카피를 `scripts/config/sector_copy.json`으로 분리했다.** 그전엔 `generate_html.py`에
  `_SECTOR_DESC` 딕셔너리로 박혀 있었는데, 섹터당 3~4문단으로 늘리면 파이썬 소스 안에 긴 한국어
  산문이 8덩어리 들어간다(§20 "정적 데이터를 코드에 리터럴로 넣지 않는다"와 같은 계열).
  `lead`(한 줄 요약) / `intro`(섹터 정의) / `composition`(종목 구성의 의미) 세 필드.
- **"이 페이지 읽는 법" 섹션 신설.** 화면이 "52주 중 N% 지점"·"외국인 보유율"을 계산해
  보여주면서 그게 왜 중요한지는 한 마디도 없었다. 이 문단은 8개 섹터가 공유하므로 카피 파일이
  아니라 템플릿에 둔다.
- **중복 콘텐츠 회피**: 공유 문단 끝에 그날 그 섹터의 실측 한 문장을 붙인다
  ("오늘 반도체 섹터는 7종목 중 2종목이 오르고 5종목이 내렸고, 섹터 평균은 -2.03%예요").
  8페이지가 완전히 같은 문단이 되지 않게 하는 장치다.
- **설명은 전부 `<details>`로 기본 접힘**(2026-09-02~03 사용자 요청). 상단은 한 줄 요약만
  남기고 "이 섹터를 어떻게 보나요"를, 하단은 "이 페이지 읽는 법"을 눌러야 펼쳐진다 —
  그 전엔 설명이 섹터 평균·종목 카드를 첫 화면 밖으로 밀어냈다. **접혀 있어도 크롤러는
  `<details>` 안 내용을 읽으므로 본문 분량에는 손해가 없다**(측정: 2,124자 → 2,138자).
- 템플릿은 `sector_copy | default({}, true)`를 쓴다 — 카피가 안 넘어와도 **문단만 빠지고 페이지
  생성은 계속된다.** 호출부 누락으로 페이지 생성 자체가 죽는 것보다 낫다.
- 재생성 후 **기존 실측 수치가 하나도 바뀌지 않은 것을 대조로 확인**했다(사라진 수치 0개,
  새로 생긴 숫자는 전부 설명문의 것). 스냅샷만 읽는 결정론적 생성이라 가능한 검증이다.
- 테스트: `scripts/test_sector_copy.py`(6) — 8섹터 카피 존재·최소 300자·intro 중복 없음·
  없는 키는 빈 dict·파일 없어도 안전·**`stock_universe.json` 섹터 키와 일치**(어긋나면 카피가
  조용히 누락된다).

**내부 링크 보강 (2026-09-03)** — 위 제거 직후 섹터 8페이지는 sitemap과 종목 상세 3곳,
서로 간의 "다른 섹터 보기" 칩으로만 도달했다(홈에서 오는 링크는 제거 **이전에도 0개**였다).

- **들어오는 링크**: `/stocks/` 푸터 안내문이 이미 8개 섹터 이름을 산문으로 나열하고 있어서
  그 이름들을 각 섹터 페이지로 링크했다. 섹터 화면(위젯)을 되살리지 않으면서 상시 진입점을
  만드는 방법이고, 문맥 안의 링크라 크롤러에게도 자연스럽다.
- **나가는 링크**: 섹터 페이지 하단에 문맥 링크 두 줄을 뒀다.
  · **밤사이 선행 지표** — `stock_universe.json`의 `bellwethers`(밤사이 브리지가 쓰는 그
    목록)를 링크 대상으로 재사용한다. **상세 페이지가 실제로 있는 것만** 나온다(§36 존재-게이트).
    현재는 semicon만 해당(NVDA·MU·SOXX). 나머지 섹터는 그 줄이 통째로 빠진다.
  · **브리핑 아카이브** — 8페이지 전부에 노출.
- 테스트: `test_sector_copy.py`에 존재-게이트 검증 2건 추가(semicon 3개 / 나머지 빈 리스트).


### 43. 브리핑 잡 실행 중에 생성물을 푸시해 마감 브리핑이 유실됐다 (2026-09-03 실사고)

**증상**: 16:25 `kospi-close-briefing` 잡이 `💾 HTML & 데이터 커밋 → main 푸시` 스텝에서
rebase 충돌로 5회 재시도 모두 실패 → §17 가드가 `exit 1`로 중단 → **그날 마감 브리핑이
발행되지 않았고**(`/briefings/2026-09-03/close/` 404) 텔레그램도 나가지 않았다.

```
CONFLICT (content): Merge conflict in web/briefings/index.html
error: could not apply 850d7fb... 🇰🇷 코스피 마감: 2026-09-03 16:32 KST
```

**원인**: 사람(작업자)이 **잡 실행 중에 같은 생성물을 푸시**했다.

| 시각 | 사건 |
| --- | --- |
| 16:25 | 잡 시작 — 그 시점 main을 체크아웃(옛 아카이브 템플릿) |
| 16:32:2x | 잡이 `write_briefings_index()`로 아카이브를 재생성해 커밋 |
| 16:32:51 | push 시도 — 그런데 main엔 16:32에 사람이 올린 **타임라인 개편**이 들어와 있었다 |

양쪽이 같은 파일을 **통째로 재생성**한 결과라 자동 병합이 불가능했다(§18과 같은 구조 —
다만 그때는 두 워크플로우, 이번은 워크플로우 vs 사람).

**구조적 문제는 아니다**: `web/briefings/index.html`을 재생성하는 워크플로우는
`daily_report.yml` 하나뿐이고(`kospi-news-live.yml`은 건드리지 않는다), 세 브리핑 잡은
시각이 겹치지 않는다. **사람이 잡 실행 중에 푸시할 때만 발생한다.**

**수정**: push 5회 실패 지점 5곳 전부에 **관리자 텔레그램 알림**을 붙였다. §32에서 배운 것이
그대로 반복됐다 — 잡은 정직하게 실패했지만 아무도 GHA를 보고 있지 않아 사용자가 35분 뒤에
직접 발견했다. 알림에 충돌 원인과 재실행 명령을 함께 적는다. 평문 발송(§37).

- **방지 룰(브리핑 잡 실행 중에는 생성물을 푸시하지 않는다)**: `web/briefings/**`,
  `web/stocks/**`, `web/sitemap.xml` 등 **워크플로우가 재생성하는 파일**을 사람이 고쳐 푸시할
  때는 브리핑 잡 시간대(**07:25~07:45 · 16:25~16:50 · 21:15~21:40 KST**)를 피한다. 생성물은
  통째로 다시 쓰이므로 3-way 병합이 성립하지 않는다 — 한 줄만 고쳐도 충돌한다.
- **방지 룰(생성물을 커밋하는 파이프라인의 대가)**: 생성물을 git에 커밋하면 리뷰·롤백은
  쉬워지지만 **동시 수정이 항상 충돌**한다. 커밋 대상에 생성물을 추가할 때는 "이 파일을
  사람이 고칠 일이 있는가"를 함께 물어야 한다. 있다면 그 순간 이 사고의 조건이 갖춰진다.
- **재발 시 진단 순서**: ① `gh run view <id> --log | grep -E "CONFLICT|could not apply"`로
  **어느 파일이 충돌했는지** 먼저 본다. ② 그 파일을 그 시각 전후에 누가 푸시했는지
...
### 45. 직전 미국장 이슈 채점 섹션 — `score_us_issues.py` (2026-09-06 신설)

미국 브리핑(21:15 KST)이 짚은 "오늘 점검할 이슈"를 다음날 코스피 아침 브리핑(07:25)에서
다시 보여주되, **그 이슈가 지목한 종목이 실제로 어떻게 움직였는지만** 붙인다. 기존 "간밤 미국 시장 이슈"(`us_issues`)
섹션을 **대체**한다 — 그 섹션은 같은 페이지 `근거`와 내용이 겹쳤다(9/4 발행본에서 2장 모두
근거·리스크 문장과 중복 확인).

- 이슈 카드는 `up`/`down` × `{label, tickers}` 구조라 **방향 주장이 검증 가능한 형태로**
  스냅샷에 남아 있다. 그 세션은 05~06시 KST에 마감하므로 07:25는 마감 1.5시간 뒤다.
- **⚠️ 적중·빗나감을 판정하지 않는다(2026-09-08 사용자 지적으로 확정).** 미국 브리핑의 이슈는
  예측이 아니라 **관전 포인트**라, 채점 대상이 아닌 것을 채점하면 무슨 말인지 알 수 없는 화면이
  된다 — "어제 랠리의 **배경**"이라는 서술형 이슈에 "3중 1 적중"이 붙어 있었다. 카드 한 줄은
  **실제 움직임만** 말한다("성장주·빅테크는 엇갈렸어요"·"나란히 올랐어요"). 어제 예상과 맞았는지는
  말하지 않고 읽는 사람이 수치를 보고 판단한다. 티커 칩 색도 판정이 아니라 **실제 등락 방향**이다.
  **적중률·배지·취소선을 다시 넣지 말 것.**
- **숫자·문구가 전부 결정론이다. LLM이 개입하지 않는다** — §0 위반 여지가 구조적으로 없다.
- 대상 세션은 `session_label.prev_us_session()`. 미국 브리핑은 아침 ET에 나가므로
  **디렉터리 날짜(KST) = 그 브리핑이 가리키는 ET 세션 날짜**로 일치한다. 월요일이면 금요일이
  잡히고 섹션 제목도 "지난 금요일 이슈, 이렇게 끝났어요"가 된다.

**⚠️ 날짜 고정 조회 — 이 스크립트에서 가장 중요한 규칙이다.**

대상 ET 세션의 봉을 골라 직전 봉과 비교한다. **그 날짜 봉이 없으면 티커를 버리고, 최신 봉으로
폴백하지 않는다.** 07:25는 미국 마감 1.5시간 뒤라 일봉이 아직 안 왔을 수 있는데, 그때 "최신 봉"을
쓰면 조용히 **전전 세션 등락률**이 실린다 — §24가 `fast_info.previous_close`를 믿어서 터진 것과
같은 계열이다. 나중에 재생성해도 같은 값이 나오는 것은 이 규칙의 부수 효과다.

**채점하지 못하는 이슈는 채우지 않는다(§0).**

| 유형 | 처리 |
| --- | --- |
| 티커 있음 (조사 시점 63%) | 해당 티커 실측 |
| "지수 전반"·"위험자산 전반" 등 지수 계열 (24%) | S&P500 프록시 + **화면에 대입 사실 명시**(§41) |
| 롱테일 라벨("원가 민감 소비재·운송" 등, 13%) | 실측 대상이 없어 카드째 생략 |

프록시 화이트리스트에 **섹터가 섞인 라벨("지수 전반, 특히 성장주")을 넣지 말 것.** S&P500 하나로
대표할 수 없다 — 매핑을 늘릴수록 실측이 아니라 추론이 늘어난다.

**⚠️ 제목의 시점 표현은 읽는 사람 기준으로 교정한다(`retitle`).** 미국 브리핑은 21:15 KST
(=그날 아침 ET)에 나가므로 제목 시제가 그 시점 기준이다 — 다음날 코스피 브리핑에서 그대로 읽으면
어긋난다("오늘 밤 고용지표"가 월요일 아침에 오늘 밤 일처럼 읽힌다). 실측 138건 기준 오늘 12·
이번 주 6·내일 5로 **미래·현재 시점어가 대부분**이고 과거형은 2건뿐이다.
- `오늘 밤`은 그 브리핑이 가리킨 **세션 자체**라 세션 라벨로 정확히 치환한다("지난 금요일 고용지표").
- 나머지(오늘·내일·이번 주·어제·간밤…)는 지금 프레임에서 뭘 가리키는지 단정할 수 없으므로
  **지어내지 않고 지운다** — 어느 세션인지는 섹션 제목이 이미 말한다(§0). 조사(은/는/이/가/의/도)도
  함께 지운다.
- 지우고 나서 6자 미만이면 원문을 유지한다. 시점을 고치려다 제목을 망가뜨리는 게 더 나쁘다.

**중복 합치기**: 같은 프록시·같은 방향이 다시 나오면 뒤엣것은 제목만 남긴다. 2026-09-03
실측에서 "고용지표 대기"와 "중동 리스크" 두 카드가 **글자 하나까지 같은 결과**를 냈다(둘 다 티커
없는 "지수 전반"). 실측은 맞지만 화면이 고장 난 것처럼 보인다. "지수 전반"이 32건으로 압도적
1위라 드문 일이 아니다.

**⚠️ [상시 룰] 미국이 쉬는 날의 다음 코스피 브리핑에는 이 섹션을 내보내지 않는다.**

직전 미국장이 **직전 코스피 브리핑보다 오래됐다면** 그 세션은 이미 어제 채점해서 내보낸 것이다.
그대로 두면 같은 결과가 이틀 연속 나간다. 미국 휴장일(노동절·추수감사절 등)에 정확히 이
상황이 만들어진다.

```
금 9/4  코스피 브리핑 → 9/3 세션 채점
월 9/7  미국 휴장(노동절). 미국 브리핑 미발행. 코스피 브리핑 → 9/4 세션 채점
화 9/8  직전 미국장이 여전히 9/4 → 이미 어제 보여준 결과 → 섹션 생략
수 9/9  직전 미국장 9/8 → 정상 채점
```

판정은 `session_label.us_session_to_score(브리핑날짜)` 하나가 소유한다 — 새로 채점할 세션이
없으면 `None`을 돌려준다. **채점 스크립트와 렌더 게이트가 같은 함수를 쓴다.** 채점 스텝이
`continue-on-error`로 실패해 어제 결과 파일이 남아 있어도 렌더에서 다시 막힌다(실측 확인함).

한국만 쉬고 미국이 열린 날은 반대 방향이라 문제가 없다 — 직전 미국장이 직전 코스피 브리핑보다
최신이므로 그대로 채점된다.

**미국 브리핑 자체는 이미 막혀 있다** — `us-briefing` 잡의 휴장 게이트가 전체 18개 스텝 중
내용 스텝 14개(수집·분석·텔레그램·이메일·HTML·커밋)를 전부 `if: steps.holiday.outputs.open ==
'true'`로 감싼다(2026-09-07 노동절로 확인). 이 섹션 규칙은 그와 별개로 **코스피 쪽**에 필요하다.

**재생성 안정성**: 결과를 `analysis["us_issue_results"]`에 주입하면 `generate_html`이 analysis를
통째로 `analysis_snapshot.json`에 저장하므로 자동으로 스냅샷에 박힌다. 재생성 시 §2 규칙대로
스냅샷이 1순위라 그 시점 시세를 다시 조회하지 않는다 — 별도 저장 장치가 필요 없다.
데이터 파일을 쓸 때는 `us_session_date`를 `prev_us_session(대상일)`과 대조하고 불일치면 버린다(§14).

**실패 모드는 전부 "섹션 생략"으로 수렴한다** — 어제 미국 브리핑 없음(휴장·발행 실패), 스냅샷에
이슈 없음, 해당 세션 봉 없음, 날짜 불일치, 스크립트 실패(`continue-on-error` + `timeout-minutes: 5`).
**어느 경우에도 틀린 숫자를 보여주지 않는다.**

- 실행: `kospi-briefing` 잡의 `fetch_data.py` 직후. 렌더 단계에 네트워크를 넣지 않기 위한 배치다(§21).
- 출력: `data/us_issue_results.json`(gitignore — 스냅샷에 흡수되는 중간물).
- 테스트: `scripts/test_us_issue_scoring.py`(21) — 판정·조사(받침 유무로 이/가)·결과줄·프록시
  매칭·카드 생략·중복 제거와 9/3·9/4 실측 리플레이. `build_cards`가 `measure`를 주입받아
  네트워크 없이 돈다.

**미해결**: 07:25에 일봉이 확정돼 있는지는 배포 전 확답 불가다. 안 왔으면 섹션이 빠지는 안전한
실패지만 매일 빠지면 기능이 없는 것과 같다 — 배포 후 며칠 로그로 확인할 것.

### 46. 코스피 주도주 타일 — 원화 등락이 %에서 역산돼 틀린 값이 나갔다 + 1분봉 곡선 제거 (2026-09-11)

**증상**: `/stocks/` "코스피 주도주" 타일의 원화 등락폭이 **세 종목 모두** 실제와 달랐다. 사용자가
보낸 스크린샷의 "▼ 9,012"는 역산 전일 종가가 269,012원이라는 뜻인데, 삼성전자는 500원 호가
단위라 **존재할 수 없는 가격**이다.

| 종목 (9/11 15:18) | 화면 | 실제 |
| --- | --- | --- |
| 삼성전자 | ▼ 9,254 | ▼ 9,250 |
| SK하이닉스 | ▼ 40,025 | ▼ 40,000 |
| 현대차 | ▼ 6,496 | ▼ 6,500 |
| 삼성전자 스냅샷 기준선(9/10) | ▼ 512 | ▼ 500 |

**원인**: `paintTile()`이 `prev = close/(1+pct/100)`로 전일 종가를 **되짚어 계산**했다. `pct`는 소수
둘째 자리에서 반올림된 값이라 오차가 가격에 비례해 커진다(SK하이닉스 +25원). 그렇게 한 이유는
`/api/stocks-live`가 네이버 원천에서 **가격과 %만 골라 넘겼기** 때문이다. 원천
(`polling.finance.naver.com`)은 실제 원화 등락(`compareToPreviousClosePriceRaw`)·시가·고가·저가·
거래대금·시총을 이미 주고 있었다. 장중만이 아니라 초기 스냅샷 기준선에서도 같은 계산을 타서
**어느 시간대든 틀린 값**이 나가고 있었다.

**수정**:
- `api/stocks-live.mjs fetchOne()` — `changeAbs`·`prevClose`(= 가격 − 실제 등락)·`open`·`high`·`low`·
  `tradingValue`(원) 추가. 기존 `price`·`changePct`는 그대로(하위호환). `fetchOne`을 named export해
  네트워크 없이 테스트한다.
- 타일은 **`changeAbs`만** 쓴다. 없으면 원화 금액을 만들지 않고 %만 보인다.
- 스냅샷 기준선의 전일 종가는 `spark20`의 **마지막 두 실제 종가**로 잡는다. 마지막 점이 종가와
  다르면 판단하지 않는다(`prevFromSpark`).
- **마감 후 추정가(HL 24h) 모드에선 원화 등락을 아예 표시하지 않는다.** 원화 금액을 만들려면 기준
  (종가)을 되짚어야 하고, 추정치에 원 단위 정밀도를 입히는 것 자체가 과장이다. "종가 대비 · 추정"
  라벨과 %만 둔다.

**1분봉 곡선 제거(같은 날, 사용자 요청)** — 선택 종목의 "오늘 장중 · 실측 1분봉" 곡선과 그 아래
"장중 지표" 블록(`#wm-curve-block`)을 뺐다. 대신 **장중 지표를 타일 안으로 올렸다**(A안 — 프로토타입
[2026-09-11-leaders-tiles.html](prototypes/2026-09-11-leaders-tiles.html)에서 두 안을 실측으로 비교).

- 타일: 현재가·실제 원화 등락·전일 종가 / 당일 레인지(저·고 + 시가 눈금) / 거래대금 / 52주 고점 대비 /
  외국인 5일 순매수(**기간 표기 필수** — 네이버 집계가 하루 늦다). 단위는 주(수량)지 원이 아니다.
- 당일 레인지는 이제 **원천의 확정 고가·저가**다. 예전엔 1분봉을 누적해 min/max를 냈다.
- 야간에는 `loadRegularSession()`이 정규장 확정 레인지·거래대금을 **한 번만** 받아 "정규장 레인지"로
  표시한다. 가격은 추정가가 담당하므로 덮어쓰지 않는다.
- **거래량 20일 평균 배수는 넣지 않았다** — 장중 누적량은 하루 치가 아니라 평균과 비교하면 항상 작게
  나온다. 잘못된 비교 대신 절대값인 거래대금만 쓴다.
- 곡선 IIFE 319줄·CSS 23규칙(+ 화살표 전용 모바일 여백)·테스트 6건(`__wmIsTodaySession`) 삭제.
  그 테스트가 지키던 교훈(2026-07-31, `/api/intraday`는 개장 전엔 전일 세션을 준다)은 **남은 두
  소비처가 각자 지닌다** — 상세 페이지 `stocks.js`(`d.date===todayKST()`)와 섹터 박스
  `sbxLoadIntraday()`.
- 국면 재배치 코드의 `if(!curve||!newsRow) return;`은 곡선이 없어지면 **야간 섹션 재배치까지 조기
  종료**시키는 구조였다. 곡선 의존만 걷어내고 섹션 순서 변경은 살렸다.
- 테스트: `api/_stocks-live.test.mjs`(4 — 실사고 리플레이·원화 등락 없을 때 null·가격 없음·빈 응답),
  `stocks-home.test.mjs` 타일 5건(원화 등락 그대로·**소스에 역산 식이 돌아오지 않는지**·스냅샷 기준선·
  레인지 위치·거래대금/수량 표기).

- **방지 룰(원천이 주는 값을 파생값에서 되짚지 않는다)**: 화면에 필요한 값을 원천이 주는데 중간
  API가 버렸다면, 클라이언트에서 다른 값으로 되짚지 말고 **API가 그 값을 넘기게 고친다.** 되짚은
  값은 원본보다 정밀도가 낮은데 원 단위로 표시되면 정밀해 보인다 — 틀린 줄 알아채기가 가장 어렵다.
  §24(분모 검증)·§30(기준가 이중 구현)과 같은 계열이다.
- **방지 룰(표시 정밀도가 데이터 정밀도를 넘지 않는다)**: 소수 둘째 자리 %에서 만든 원화 금액을
  1원 단위로 보여주는 것은 없는 정밀도를 만들어내는 것이다. 호가 단위가 있는 가격이라면, **표시된
  값이 호가 단위에 맞는지**가 가장 빠른 진단이다.
- **배포 사고(같은 날)**: 처음엔 테스트를 `api/stocks-live.test.mjs`로(접두 `_` 없이) 만들었다가
  Vercel이 그걸 **13번째 서버리스 함수**로 배포하려다 Hobby 한도(12개)에 걸려 프로덕션 배포가
  실패했다. CI는 통과해 푸시 전엔 신호가 없었다. `api/_stocks-live.test.mjs`로 옮기고
  `api/_route-budget.test.mjs`를 추가했다 — 테스트 파일이 라우트가 되거나 라우트가 12개를 넘으면
  CI가 실패한다. **`api/`에 파일을 추가할 때는 `_` 접두 여부가 곧 배포 여부다.**
- **재발 시 진단 순서**: ① 화면의 가격 + 등락으로 전일 종가를 역산해 **호가 단위에 맞는지** 본다.
  ② 맞지 않으면 그 값을 만드는 코드가 %에서 되짚는지 확인한다. ③ 원천 응답에 실제 원화 등락 필드가
  있는지 확인하고, 있으면 중간 API가 넘기게 고친다.

### 47. 네이버 리서치 게시판 이전 — 목표주가 3종목 0건 + 증권가 시황 0건 (2026-09-11 실사고, 수정 완료)

**증상**: `/stocks/` 허브의 "🎯 증권사 목표주가"가 3종목 모두 "리포트 0건"으로 비었다(사용자 발견).
같은 원인으로 마감 브리핑의 "오늘 증권가 시황"(`fetch_research_reports.py`)도 16:25에 0건이 될
상황이었다.

**원인**: 네이버가 `finance.naver.com/research/*.naver` 게시판을 `stock.naver.com/research/*`
(Next.js, 클라이언트 렌더)로 옮겼다. 옛 URL은 302로 새 페이지에 넘어가고, 종목 목록은
`itemCode` 필터까지 떨어진다. 새 페이지의 HTML에는 표가 없어(`<tr>` 0개) 옛 정규식이 **0행을
읽고도 예외 없이 정상 종료**했다. 07:27 수집이 멀쩡하던 `stock-targets.json`을 리포트 0건으로
덮어썼다.

**왜 기존 가드가 못 잡았나**: §20의 신선도 가드(`_targets_data_is_stale`)는 `updated_at`만 본다.
수집기는 정상 종료하며 **오늘 날짜로** 빈 파일을 썼으므로 "신선한 0건"으로 통과했다. "수집이
죽었나"와 "수집이 빈손을 오늘 날짜로 썼나"는 다른 질문이다(§39의 CNN `timestamp`와 같은 함정).

**수정**:
- `fetch_stock_targets.py` — 새 페이지가 쓰는 JSON API로 전환.
  목록 `m.stock.naver.com/api/research/stock/{code}`, 상세 `/api/research/company/{researchId}`.
  목표가·투자의견이 `goalPrice`·`opinion` **구조화 필드**로 온다(옛 방식은 상세 HTML 산문을 정규식으로
  긁었다). 옛 nid와 researchId는 같은 번호 공간이다(93991 = 하나증권 삼성전자 7/8로 확인).
- **목록에서 다른 종목 리포트를 버린다.** 같은 계열 `/api/research/company?itemCode=`는 필터를
  **조용히 무시하고** 전 종목을 돌려줬다. 필터가 풀리면 남의 목표가가 컨센서스에 섞인다.
- **3종목 모두 목록 0건이면 파일을 덮어쓰지 않는다** — 관리자 알림 + `exit 1`(스텝은
  `continue-on-error`라 브리핑은 계속된다). 대형주 3종목이 동시에 리포트 0건일 수는 없으므로
  이건 "리포트가 없다"가 아니라 "원천을 못 읽었다"다. 덮어쓰지 않으면 `updated_at`이 멈춰 §20
  가드가 5일 뒤 섹션을 내린다 — 가드가 설계대로 작동할 수 있게 해주는 것이다.
- `fetch_research_reports.py` — `m.stock.naver.com/api/research/market`(목록)·`/market/{id}`(상세)로
  전환. 필터 규칙(당일·국내 키워드·재게시 제외·증권사당 1건·숫자 잔존 폐기)은 그대로다.
- 실측: 목표주가 로컬 실행 결과가 9/10 파일과 **완전히 일치**(삼성 485,833·12개사, SK하이닉스
  3,350,000·13개사, 현대차 733,333·12개사). 시황은 당일 후보 2건·요약 정상.
- 테스트 `test_fetch_stock_targets.py`(17) — JSON 픽스처로 교체, 다른 종목 혼입 차단·깨진 행·
  **전 종목 0건 시 기존 파일 유지(실사고 리플레이)** 추가.

- **방지 룰(스크래퍼의 "0건 정상 종료"를 의심한다)**: HTML 스크래퍼는 원천 구조가 바뀌면 예외 없이
  0건을 돌려준다. 파서가 행을 하나도 못 찾았을 때 그게 "진짜 0건"인지 "못 읽음"인지 구분할 근거를
  미리 정해둔다 — 여기선 "대형주가 전부 0건일 리 없다". 그 조건이면 **쓰지 않는다.**
- **방지 룰(빈 결과로 좋은 데이터를 덮어쓰지 않는다)**: 원자적 쓰기(`_write_atomic`)는 반쯤 쓴 파일을
  막지만 **틀린 파일을 완전하게 쓰는 것**은 막지 못한다. 수집 결과가 구조적으로 불가능한 모양이면
  직전 파일을 유지한다.
- **방지 룰(같은 원천을 쓰는 수집기를 함께 점검한다)**: 원천 이전은 한 수집기만 깨지 않는다. 이번에도
  목표주가를 조사하다 같은 게시판 계열의 시황 수집기를 발견했다. 원천이 바뀌면 `grep`으로 그 도메인을
  쓰는 스크립트를 전수 확인한다.
- **남은 갭**: 다른 `finance.naver.com` 소비처는 이번 이전 대상이 아니었지만
  (리서치 게시판만 이전) 같은 방식으로 깨질 수 있다. 0건 가드가 없는 수집기는 같은 사고를 조용히 낸다.
- **재발 시 진단 순서**: ① 수집 로그에 예외가 없는데 결과가 0건이면 원천 URL을 `curl -I`로 열어
  302 여부를 본다. ② 리다이렉트 목적지가 클라이언트 렌더 페이지면, 그 페이지가 부르는 JSON API를
  찾는다(`m.stock.naver.com/api/research/{stock|company|market|invest}`). ③ 필터 파라미터가 실제로
  먹는지 **결과의 itemCode로** 확인한다 — 200 응답이 필터 적용을 뜻하지 않는다.

### 48. 마감 잡이 애프터장 가격을 종가로 저장 + 토스 일봉이 공식 종가가 아니었다 (2026-09-14 실사고, 수정 완료)

**증상**: KRX 애프터마켓(16:00~20:00)이 처음 열린 날, 16:25 마감 잡이 저장한 `stocks-snapshot.json`의
SK하이닉스 종가가 **1,693,000(−6.57%)**, 현대차 **370,000(−3.27%)**였다. 공식 종가는 **1,697,000(−6.35%)**·
**371,500(−2.88%)**이다. `stock-targets.json`의 상승여력 기준가도 1,694,000·370,000으로 틀렸다.
'어제랑 비교해서' 프로토타입 데이터를 서로 다른 출처와 대조하다 발견했다.

**원인 1 — 오늘 봉이 장 마감 뒤에도 움직인다(9/14부터)**: 15:30 뒤 네이버 일봉의 오늘 종가, 네이버 실시간
`closePrice`, 토스 일봉이 **애프터장 체결가를 따라 계속 바뀐다**(SK하이닉스 16:54 1,693,000 → 17:10 1,688,000).
마감 잡은 애프터장 한복판(16:25~16:35)에 돌아 그 순간의 애프터장 가격을 종가로 박았다.
정규장 공식 종가를 주는 곳은 **네이버 15:30 1분봉**(야후 `.KS` 종가와 일치)뿐이었다.

**원인 2 — 토스 일봉은 원래부터 공식 종가가 아니었다**: 5종목(삼성전자·SK하이닉스·현대차·NAVER·LG에너지솔루션)
30거래일을 대조하니 토스 일봉 종가가 **26~29일** 공식 종가와 달랐다(최대 **8.27%**, SK하이닉스). 같은 기간
네이버 일봉은 야후와 **0일** 달랐다. 토스 캔들 API에는 거래소·세션을 고르는 옵션이 없다. 넥스트레이드·시간외
체결이 섞인 값으로 보인다. §37이 "토스·네이버 최대 4% 차이 — 별도 조사 필요"로 남겨둔 미해결 항목의 원인이다.
이 서비스는 한국 종목 실측 1순위를 토스로 두고 있었으므로, 브리핑 픽 가격·20일선·손절 검증(§41)·목표주가
상승여력이 **그동안 계속** 공식 종가와 어긋나 있었을 수 있다.

**수정**:
- `scripts/kr_official_closes.py` 신설 — 정규장 공식 종가 일봉의 **단일 소스**(§30). 과거는 네이버 일봉,
  15:30이 지난 오늘 봉만 15:30 1분봉 `currentPrice`로 교체한다. 1분봉을 못 구하면 **오늘 봉을 뺀다**(틀린 값을
  쓰지 않는다). 15:30 전(장중) 봉은 건드리지 않는다 — 진행 중인 봉은 기존 pin(§37)이 맡는다.
- `validate_analysis._fetch_kospi_realdata` — 토스를 빼고 이 헬퍼만 쓴다(브리핑 픽·목표주가·상세 페이지 공통).
- `build_stocks_snapshot._naver_day_rows` — 헬퍼로 위임(행 모양이 같아 소비처 변경 없음).
- `generate_html._fetch_stock_closes`(52주 범위) — 토스 대신 헬퍼. 안 쓰이게 된 토스 import·`_naver_daily_closes` 제거.
- `api/stocks-live.mjs` — 장이 닫힌 뒤엔 실시간 값 대신 **1분봉으로 정규장 시·고·저·종**을 만든다
  (`regularSessionFromBars`·`fetchClosedSession`). 실시간 `lowPrice`도 애프터장이 섞였다(9/14 저녁 삼성전자
  248,000 vs 정규장 저가 248,500). 정규장만의 거래대금은 원천에 없어 비운다. 등락도 비운다 — 새벽 프리마켓이
  열리면 실시간 '전일 종가'가 다음 날로 넘어간다. 주도주 타일의 야간 "정규장 레인지"가 이제 실제 정규장이다.
- `api/hl-night.mjs`(야간·주말 추정가) — 앵커 `KRX 종가 × (HL 현재가 ÷ 종가 시점 HL가)`의 종가가 실시간
  `closePriceRaw`였다. 장이 닫힌 뒤엔 애프터장 가격이라 **원화 추정가가 흔들린 종가 위에 곱해졌다**(9/14 17:22
  SK하이닉스 1,669,357 → 공식 종가 기준 1,683,183). 등락률(비율)은 종가와 무관해 맞았다. 1분봉 정규장 로직을
  `api/_kr-regular-session.mjs`로 옮겨 stocks-live와 **공유**하고(§30), `pickKrxClose()`가 장이 닫혔으면 정규장
  종가를, 그 정규장 날짜가 앵커 봉과 다르면 null을 준다(애프터장 가격으로 폴백하지 않는다).
- 테스트: `api/_hl-night-core.test.mjs` 4건 추가(9/14 SK하이닉스 리플레이·날짜 불일치·조회 실패·장중),
  `scripts/test_kr_official_closes.py`(7 — 실사고 리플레이·1분봉 없음/실패 시 오늘 봉 제외·장중 불변·
  **`_fetch_kospi_realdata`가 토스를 부르지 않는지**), `api/_stocks-live.test.mjs` 4건 추가(9/14 삼성전자 실측).
- 실측 확인(17:18): 헬퍼·`_fetch_kospi_realdata`·API가 세 종목 모두 공식 종가(249,000·1,697,000·371,500)를 낸다.

**확인하다 바로잡은 내 판단**: 처음엔 실시간 SK하이닉스 `lowPrice` 1,686,000을 애프터장 저가로 봤는데, 야후와
1분봉으로 대조하니 **15:14 정규장 저가**였다. 애프터장이 섞이는지는 종목·시점마다 다르다 — 가격이 "그럴듯하게
틀렸는지"는 짐작하지 말고 공식 출처와 대조해 확정한다.

- **방지 룰(공식 종가의 정의를 코드에 박는다)**: "일봉 종가 = 종가"라는 전제는 거래 세션이 늘어나는 순간 깨진다.
  넥스트레이드(2025)와 KRX 애프터마켓(2026-09-14)으로 **한 날짜에 가격이 여러 개**가 됐다. 한국 종목의 종가는
  "KRX 정규장 15:30 종가"로 정의하고, 그 정의를 `kr_official_closes` 한 곳에서만 구현한다.
- **방지 룰(1순위 소스도 공식 출처와 주기적으로 대조한다)**: 토스는 "공식 API라서 정확하다"는 전제로 1순위였지만
  한 번도 공식 종가와 대조된 적이 없었다. §37이 차이를 발견하고도 "별도 조사"로 미뤘다. 소스를 1순위로 올릴 땐
  며칠치를 독립 출처(야후 `.KS` 등)와 대조한 결과를 함께 남긴다.
- **방지 룰(마감 뒤 잡은 '지금 값'이 아니라 '확정 값'을 읽는다)**: 16:25 마감 잡처럼 세션이 끝난 뒤 도는 잡은
  실시간·당일 봉 필드를 읽지 않는다. 그 필드는 이제 다음 세션(애프터장)의 값이다.
- **9/14 20:12 실측으로 확정 — 네이버 일봉은 지난 날짜도 공식 종가가 아니다.** 9/14 봉이 20:00 애프터장 가격으로 굳었다
  (삼성전자 248,500 / 공식 249,000, SK하이닉스 1,683,000 / 1,697,000, 현대차 367,500 / 371,500). 실시간 `closePriceRaw`도 같은 값이다.
  같은 날 밤 `kr_official_closes`를 고쳤다 — **2026-09-14 이후 봉은 전부** 15:30 1분봉 공식 종가로 바꾸고,
  확인한 값은 `data/kr_official_closes.json`에 쌓는다(1분봉은 약 7거래일만 조회된다). 저장은 마감 잡의
  `build_stocks_snapshot.py`만 한다(§18). 저장소에도 1분봉에도 없는 봉은 **뺀다**(틀린 종가를 쓰지 않는다).
  테스트 `scripts/test_kr_official_closes.py`에 지난 날짜 교정·저장소 우선·없으면 제외·저장 4건 추가.
- **남은 갭**: (a) ~~오늘 봉이 20:00 뒤 공식 종가로 확정되는지~~ → 확정되지 않는다(위). 저장 잡이 7거래일 넘게 멈추면
  그 사이 날짜는 복구할 수 없다. (b) 오늘 봉의 거래량
  (`accumulatedTradingVolume`)에도 애프터장이 섞였을 수 있다(스냅샷 vol·vol_avg20). (c) 미국 종목·지수의 토스
  경로는 이번에 대조하지 않았다.
- **재발 시 진단 순서**: ① 화면·파일의 종가를 야후 `{code}.KS` 종가와 대조. ② 다르면 네이버 15:30 1분봉과 대조 —
  1분봉과 야후가 같으면 소스가 애프터장·통합 가격을 준 것이다. ③ 어느 코드가 `kr_official_closes`를 우회해
  일봉·실시간·토스 종가를 직접 읽는지 grep한다.


- **9/15 후속 감사 — 15:30 뒤 애프터장 값을 정규장 값처럼 쓰던 나머지 경로** (9/14 1분봉·일봉 대조 + 9/15 16:04·16:15·16:40·17:37 실측)

  | 경로 | 판정 | 조치 |
  | --- | --- | --- |
  | 일봉 거래량 → 스냅샷 `vol`·`vol_avg20`, dpick 20일 평균 | **오염.** 9/14 애프터장 거래량 비중 삼성전자 6.7%·SK하이닉스 6.4%·현대차 5.8%·**한미반도체 20.1%**·KB금융 1.9%. 네이버 fchart·siseJson·pykrx·야후 거래량도 같은 값이라 정규장 거래량을 주는 일봉 소스가 없다 | `kr_official_closes`가 9/14 이후 봉 거래량을 **09:00~15:30 1분봉 합**으로 바꾸고 `volumes`로 저장한다. 못 구하면 None(원본을 남기지 않는다). 스냅샷은 오늘 거래량이 없으면 전날 거래량을 오늘 값으로 쓰지 않는다. dpick이 따로 하던 합산은 `official_session()`으로 합쳤다(§30) |
  | `/api/signals` 장 마감 뒤(특이 신호 `phase: 'closed'`) | **오염.** 9/15 17:37 현대차 실시간 -0.94% vs 공식 -1.21%, SK하이닉스 -0.47% vs -0.41% | `_kr-regular-session.fetchLastRegularSession()` — 직전 거래일 15:30~그날 15:30 1분봉 한 번으로 종가·전일 공식 종가·정규장 거래량. 세션 날짜가 같으면 인스턴스 메모리에서 다시 쓴다. 15:30 봉이나 전일 15:30 봉이 없으면 그 종목을 뺀다. `asOf`는 스냅샷 날짜가 아니라 그 세션 날짜 |
  | `fetch_movers_why.py`(16:25 마감 잡, 16:35~23:30 매시) | **오염.** 실시간 등락률·거래량 | 15:30 뒤엔 `kr_official_closes.official_today()` — 공식 종가끼리의 등락률과 정규장 거래량. 오늘 봉이 없으면 건너뛴다 |
  | `fetch_valuation.py`(16:25) PER | **오염.** integration PER은 실시간 가격 ÷ EPS다(EPS가 그대로인데 75초 사이에 값이 바뀌었고, 16:04→16:40에 애프터장을 따라 7.52→7.53) | 정규장 밖에선 `regular_close() ÷ EPS`로 다시 계산한다(반올림된 EPS로도 원천 PER이 재현된다 — 248,500 / 22,292 = 11.15). 종가를 못 구한 종목은 PER을 비우고, 한 건도 못 구하면 기존 파일을 덮지 않는다. `asOf`는 그 세션 날짜 |
  | 마감 잡 `kospi200_top10`(marketValue 목록)·`ai_semicon_stocks` 국내 2종목(pykrx) | **오염.** 16:04 목록 SK하이닉스 1,687,000·-0.59%. 렌더되진 않지만 마감 프롬프트에 파일 전체가 들어간다(`call_claude` 마감 경로) | 09:00~15:30 밖에선 `official_today()`. 못 구한 종목은 뺀다 |
  | 픽 채점·스파크라인 날짜(`generate_html._naver_dated_rows`) | 원본 일봉 종가 사용 | `official_day_rows()`로. 스냅샷 스파크라인 종가와 날짜가 같은 봉 집합이 된다 |
  | `api/vol-top.mjs`, `build_etf_exposure.py` | 오염이었지만 §52에서 삭제됐다 | — |
  | 업종·급등주·dpick 수급·마감 수급 | §51이 처리했다 | — |
  | 코스피 지수(일봉·실시간·`/api/market`) | **영향 없음.** 9/14 지수 1분봉이 15:32에 멈추고 일봉 종가 6,684.37과 같다 | 그대로 |
  | 시장 폭(`upDownStockInfo`) | 영향 없음(§51 실측) | 그대로 |

  - 1분봉 `accumulatedTradingVolume`은 이름과 달리 **봉 하나의 거래량**이다(9/14 SK하이닉스 09:00~15:30 합 3,742,905 + 15:31~15:59 957 + 애프터장 256,370 ≈ 일봉 4,001,549). 9/11(애프터장 이전) 일봉과 정규장 합의 차이는 0.04~0.2%(시간외 거래)라 과거 봉과 정의가 사실상 같다.
  - 일봉 시가·고가·저가도 오염돼 있다(9/14 저가 1,678,000 / 정규장 1,686,000). 지금 그 필드를 읽는 파이썬 소비처가 없어 고치지 않았다 — 새로 읽게 되면 1분봉에서 만든다.
  - **섹터 박스 마감 직후(2026-09-23 발견)**: 15:30 뒤 마감 잡이 스냅샷을 갱신하기(~16:31) 전엔 카드·머리 라벨이 어제 스냅샷인데 탭 통계·한 줄 요약은 `/api/signals`의 오늘 값이라 한 박스에 두 날짜가 섞였다(카드 3상승 4하락 vs 통계 "상승 6 하락 1"). 이 구간엔 `/api/stocks-live` 마감 세션의 오늘 공식 종가 ÷ 스냅샷(어제 공식 종가)으로 카드를 맞춘다(`sbxClosedOverlay`). 스냅샷이 직전 거래일이 아니거나 조회에 실패하면 오늘 신호 값을 쓰지 않고 한 줄 요약을 숨긴다.
  - 테스트: `test_kr_official_closes.py`(거래량 교정·저장소·`official_today` 경로), `test_snapshot_regular_volume.py`, `test_valuation_regular_close.py`(`last_closed_session` 포함), `test_close_quotes_regular.py`, `test_movers_why.py`, `api/_kr-regular-session.test.mjs`.
  - **남은 갭**: (a) 모든 종가·거래량 교정은 1분봉 조회 기한(약 7거래일)에 묶여 있다. 저장소는 스냅샷 유니버스 46종목만 쌓이므로 dpick처럼 그날그날 바뀌는 종목은 오래된 날의 거래량을 복구할 수 없어 평균 일수가 짧아진다(§51의 5일 하한). (b) 장중 대결판 `_vs-prices.prevClose()`는 15:30 봉 조회를 따로 구현한다 — 같은 값이지만 코드가 둘이다. (c) `fetch_news_live.py`의 시장 실측 대조는 토스 일봉 전일 종가를 쓴다(장중 게이트라 애프터장과는 무관하지만 §48의 토스 문제는 남아 있다). 수집은 §53으로 멈춰 있다.
...
- **짧은 키워드의 오탐은 사전의 `exclude`로 막는다.** '유가'가 '유가증권', '기관'이 '위기관리'에 걸리던 것처럼 무관한 합성어는 `exclude: { 키워드: [합성어] }`에 넣는다. 값은 반드시 배열이다 — 문자열을 넣으면 글자 단위로 지워 매칭이 망가진다.
- **이슈 비교는 지금 꺼져 있다(C2, 2026-09-14).** 이슈 아카이브가 6개 상한으로 잘리고 최신 항목의 시각이 갱신 때마다 덮어써져(§10) "어제 이 시각까지"를 보장하지 못한다. 응답의 `issues`는 항상 null — 계산 코드(`_vs-issues.mjs`)와 사전은 남겨두고, 불변 장중 기록이 생기면 다시 켠다.
- **주도주 평균은 3종목이 모두 있을 때만.** 한 종목이라도 비면 칸을 그리지 않는다.
- **비교 시각은 확정된 같은 분 하나다(2026-09-15).** 지금 분의 진행 중인 1분봉과 **직전 분의 봉도 버린다** — 9/15 13:30 봉을 5초 간격으로 다시 읽으니 끝난 뒤 약 1분 동안 응답마다 값이 오갔다(코스피 6,627.19↔6,632.57). 그렇게 굳은 봉들 중 코스피·주도주 3종목·수급 표가 모두 가진 가장 이른 분으로 맞춘다. 화면은 2분쯤 늦게 따라온다. 한 원천만 2분(`LAG_MIN`) 넘게 늦으면 전체를 끌어내리지 않고 그 칸만 비운다. 9/15 10:11 실측에서 진행 중인 봉과 종목마다 다른 분이 섞여, 오늘 코스피 −0.02%(확정 −0.16%)·주도주 어제 평균 −4.40%(같은 분 −4.31%)가 나갔다.
- **주도주 비교는 대결판 카드의 표가 아니라 각 주도주 타일(`#us-linked-widget`) 안 '어제 같은 시각' 블록(`.vs-tb`)에 그린다.** 블록 머리에 비교한 확정 분을 적는다 — 타일 위 큰 가격은 실시간이라 블록의 '오늘'과 다를 수 있다. 1분봉 가격은 넥스트레이드 체결이 섞인 통합 가격(250,250원 같은 중간가)이고 기준 종가는 KRX 공식 종가다.
- **코스피 곡선은 차이를 색 띠로 보여준다(A안, 2026-09-15).** 같은 분에 두 곡선 값이 모두 있는 구간만 채우고(오늘이 위면 빨강·아래면 파랑), 선 끝에 값·차이 괄호, 왼쪽에 세로 눈금, 지금 선과 남은 장 음영을 둔다. SVG는 가로로 늘려 그리므로 글자는 넣지 않고 같은 비율(%)의 HTML로 겹친다. 두 끝 값이 가까우면 괄호를 빼고 라벨끼리 밀어낸다. 시안 `docs/prototypes/2026-09-15-vs-chart-variants.html`.
- **곡선을 H안으로 바꿨다(2026-09-17).** 두 곡선을 매끄럽게 그리고 그 사이를 칠한다 — 오늘이 어제보다 위면 빨강, 아래면 파랑(같은 날 잠깐 올렸던 D안은 면을 0% 기준으로 칠해 색이 '어제 대비'를 뜻하지 않았다). 같은 분에 어제 값이 없으면 칠하는 구간을 끊고(§0), 부호가 바뀌는 두 점 사이는 직선 교차점을 그림 경계로만 쓴다. 두 선 차이는 곡선 아래 칸에 **샘플 분마다 0축 세로 막대**(오늘−어제)로 그리고 끝에 지금 차이 %p를 적는다. 곡선 안 −%p 글자는 남은 장 칸에만 두고(왼쪽으로 넘기면 곡선을 덮는다) 장 끝 무렵·모바일에선 빼며, 아래 칸 끝 값이 대신한다. 매끄러운 곡선은 샘플 점을 지나지만 점 사이에서 살짝 넘칠 수 있다 — 값은 마우스 오버가 실제 샘플 점만 보여준다. 시안 `docs/prototypes/2026-09-17-vs-chart-styles.html`·`2026-09-17-vs-diff-color.html`.
- **뒤에서 열린 탭은 보이는 순간 바로 불러온다(2026-09-15).** 숨은 탭에서는 요청을 건너뛰는데(2026-08-16 차단 사고), 예전엔 다음 60초 폴링까지 대결판이 비어 있었다. `visibilitychange`에서 보이게 되면 즉시 `load()`를 부른다.
- **수급은 코스피 시장 전체 합계다.** 주도주 표 아래에 붙어 3종목 수급으로 읽혀서 라벨을 '코스피 전체 · 투자자별 누적 순매수'로 바꾸고 안내 한 줄을 붙였다.
- **어제 데이터는 인스턴스 메모리에서 재사용한다(2026-09-15).** 어제 1분봉·전일 종가·어제 수급 표는 장중에 바뀌지 않으므로, 조립 1회당 네이버 요청이 26건에서 8건으로 준다(실측). 빈 응답·실패는 기억하지 않고 다음 조립에서 다시 부른다. 날짜가 바뀌면 통째로 비운다.
- **사용량 감시가 매일 돈다(2026-09-15).** `.github/workflows/usage-snapshot.yml`이 KST 12:10에 `scripts/usage_snapshot.py`를 돌려 `data/vercel-usage-log.json`에 쌓고, 폴링 API가 no-store로 돌아갔거나 팀이 차단되면 관리자에게 알린다. 2026-08-17 수동 실행 뒤로 멈춰 있었다. 대결판 API도 감시 목록에 있다.
...
### 50. 자금 지도(ETF 자금흐름) 제거 (2026-09-15 결정)

사용자 요청으로 `/stocks/`의 **자금 지도를 데이터 수집까지 통째로 뺐다.** 서브탭은 `전체`·`특이신호` 2개가 됐다.

- 지운 것: 서브탭 `flow`, 홈 블록 `#flow-block`, 화면 `#flow-map`, `flow-map.js/css/test`, `stocks-home.js`·`.css`의 자금 지도 블록,
  수집 워크플로 `etf-flows.yml`, `build_etf_flows.py`·`build_flow_map.py`와 테스트, 데이터 `web/data/etf-flows.json`·`flow-map.json`·`data/etf_flow_history.json`,
...
- **업종·급등주**: 원천에 정규장 전용 값이 없다. 9/15 실측(15:58 → 16:12)에서 업종 79개 중 71개의 등락률이 바뀌고
  목록 현재가도 움직였다(010580 2,790 → 2,770). 그래서 **16:00 이후(또는 09:00 전) 실행에선 조회하지 않고 비운다.**
  16:25 마감 잡에선 두 섹션이 매일 빈다 — 틀린 값보다 낫다(§0). 정규장 값을 다시 채우려면 15:31~15:59에 수집하거나
  종목별 15:30 1분봉으로 다시 계산해야 한다(미구현).
- **시장 폭**: `upDownStockInfo`는 15:36·15:58·16:12 모두 254/623/40으로 같아 애프터장 가격을 따르지 않았다. 그대로 쓴다.
- **체결 시각으로는 판정할 수 없다**: 목록의 `localTradedAt`은 체결이 없어도 조회 시각으로 갱신된다(15:58 조회에서
  가격 변화 없이 15:58). 처음엔 이 값으로 가드를 만들었다가 실측으로 버렸다.

**가드(§47 패턴)**:
- 거래일에 비어 있을 수 없는 네 항목(`market_breadth`·`sectors`·`top_gainers`·`investor_trading`)이 비거나
  dpick 유니버스가 0건이면 **관리자 텔레그램 알림**. 시간대 때문에 일부러 비운 업종·급등주는 알림에서 뺀다. 발행은 멈추지 않는다 — 섹션이 빈 브리핑이 틀린 브리핑보다 낫다(§0).
...
- **재발 시 진단 순서**: ① 마감 잡 로그에서 `[fetch_closing]` 줄의 빈 항목과 `16:00 이후엔 목록 가격이 애프터장 값이라 비운다`
  문구를 구분한다(전자는 원천 고장, 후자는 시간대 가드). ② 원천 URL을 `curl -s -o /dev/null -w '%{http_code}'`로 연다.
...
### 53. 홈 장중 이슈 피드 제거 + 장중 이슈 수집 중지 (2026-09-15 결정)

사용자 요청으로 `/stocks/` 맨 위의 **장중 이슈 피드(`#bc-feed`)와 그 아래 '오늘 예측' 부줄(`#bc-sub`)을 뺐고, 장중 이슈 수집을 멈췄다.**

- **§13 타임테이블이 바뀐다.** 09:00~16:30의 `issue` 슬롯이 없어지고 07:30~16:30 내내 코스피 예측 카드를 보여준다. 마감(16:30~)·미국 예측(21:20~) 슬롯과 "오늘 ready일 때만" 규칙은 그대로다.
- **수집 중지(§10)**: `kospi-news-live.yml`에서 `fetch_news_live.py` 스텝과 `kospi-news-*.json` 커밋 경로를 뺐다. 같은 워크플로의 '왜 움직였나'(`movers-why-*`)·'종목별 관련 뉴스'(`stock-news.json`)는 주도주 위젯이 쓰므로 그대로 돈다. `daily_report.yml`의 옛 `kospi-news-live` 잡은 `if: false &&`로 막았다.
- **API**: `/api/data`의 `news-live` 허용 키를 지웠다(다른 키 `briefings-list`·`movers-why`는 그대로). 라우트 자체는 남는다.
- **확인한 소비자**: 브리핑 페이지(`main.js`)는 이 데이터를 더 이상 읽지 않았고(주석만 남아 있었다), 장중 대결판 이슈 비교는 §49 C2로 이미 꺼져 있다. 그래서 수집 중지의 영향은 홈 피드뿐이다.
- **남긴 것**: `scripts/fetch_news_live.py`와 테스트, 이미 쌓인 `web/data/kospi-news-*.json` 아카이브. 되살릴 때 워크플로 스텝만 되돌리면 된다.
- 계기가 된 화면에서 09:30 항목의 원문이 **전날 15:34 기사**였다(링크의 기사 번호 `2026091415342847890`) — §29의 "장이 열린 뒤 발행분만" 게이트를 통과한 경로는 따로 확인하지 않았다.

### 54. 대결판 축 확장 — 강도·주도권·수급 (2026-09-16~17)

§49 대결판이 "가격" 한 축(코스피 등락률·외국인 누적 순매수·주도주 3종목 평균)뿐이던 것에 세 축을 더했다.
**강도**(누적 거래량·일중 진폭), **주도권**(코스피200·코스닥 + 섹터 대표 3종목 평균·순위 이동),
**누가**(개인·외국인·기관 + 기관 세부 6칸: 금융투자·연기금·투신·은행·기타금융·보험). 새 파일
`api/_vs-axes.mjs`·`api/_vs-sectors.mjs`·`api/_vs-close.mjs`, 기존 파일 수정 `api/_vs-intraday.mjs`·
`api/_vs-flow.mjs`·`api/_vs-prices.mjs`·`api/intraday.mjs`(`?vs=close` 분기, **새 라우트 아님** —
11개 유지), `web/assets/vs-yesterday.js`·`.css`. 설계는
`docs/superpowers/specs/2026-09-16-vs-axes-design.md`.

**시간대별 카드(사용자 확정)**: 장 전(07:30~09:00) 카드 없음 — 어제 확정값은 전날 밤 이미 보여줬다.
장중(09:00~15:30)은 결론(곡선)·강도·주도권·수급 4장(`vs=intraday`). **마감 후(15:31~오늘 미국 브리핑 발행 전)는 메인 곡선(결론·곡선·근거 줄)만**(`vs=close`) — 2026-09-17 결정, 2026-09-22에 미국 브리핑 발행까지 늘렸다. 발행 여부는 21:00부터 `briefings-list.json`의 오늘 `us.state === 'ready'`로 판정하고(60초 폴링마다), 발행이 없는 날(미국 휴장)은 자정까지 남는다. 17:00에 '밤사이 미국 반도체 시황'이 위로 올라오면 곡선은 **섹터별 대표 종목 바로 위**로 내려간다(`stocks-home.js` 국면 재배치).
**미국 브리핑 발행 뒤~07:29는 카드 없음**(2026-09-17 사용자 결정 — 처음엔 강도 1장을 남겼지만 뺐다). 대결판 API도 부르지 않는다. 주말은 카드 없음. `slotOf`는 로컬
getter/offset 대신 `shouldPoll`과 같은 UTC+9 방식으로 짰다 — KST 밖 시청자에서도 슬롯이 안 틀어진다.

**대결판 결론 카드의 근거 줄(`.vsx-why`)은 오늘 코스피 등락률을 다시 적지 않는다.** LIVE 지수바가
같은 지표를 이미 보여주는데 근거 줄까지 적으면 한 화면에서 숫자가 갈린다 — 장중엔 LIVE가 실시간,
대결판이 확정 1분봉이라 약 2분 시차(13:24~13:30 사이 +1.04~+1.24%), 마감 후엔 LIVE 수급에 애프터장이
섞인다(§51 남은 갭, 9/15 확정 −11,863/+12,116/−16,825 vs LIVE −11,820/+12,173/−16,861). 값이 틀린 게
아니라 시점이 다른 것이다(13:28 LIVE `+1.10%`는 확정 13:28 봉과 정확히 일치). 그래서 근거 줄의 "가격"
항목은 "어제 이 시각엔 −0.78%였어요. 지금은 +1.81%p 더 높아요" 형태만 쓴다 — 오늘 절대값은 LIVE
바가 단독으로 담당한다. **이 규칙은 근거 줄 중 "가격"에만 해당한다** — "강도"·"주도권"·"누가"는
코스피 등락률이 아니라 거래량·진폭·기관 순매수라 승인 시안대로 오늘 절대값도 함께 적는다(예:
"거래량이 +24.05%(어제 63,729천주), 진폭은 1.26%(어제 1.32%)"). 마감 후·밤에는 수급 심층 카드
자체의 라벨을 "새 축 · 수급 심층"에서 "**정규장 확정 15:40**"으로 바꿔, 그 카드 숫자가 애프터장이
섞인 LIVE 바 수급과 다른 기준(정규장 확정)임을 밝힌다(`flowCard(axes, slot)`).

**결론 제목은 오늘의 방향을 먼저 말한다(2026-09-22 사용자 지적).** "오늘이 어제보다 약해요"만 쓰면, 어제와 오늘 모두 오른 날
(오늘이 덜 오른 날)에 지수가 떨어진 것으로 읽혔다. 그래서 `_vs-core.verdict()`가 오늘·어제 등락률로 문장을 고른다 —
"어제보다 더 오르고 있어요" · "오르고 있지만 어제보다 오름폭이 작아요" · "어제와 달리 내리고 있어요" · "어제처럼 오르고 있어요" 등.
마감 후엔 과거형("올랐지만 … 오름폭이 작았어요"). 근거 줄(`vs-sub`)엔 **오늘·어제 값을 함께** 적는다(`코스피 오늘 +2.05% · 어제 같은 시각 +1.18% — 0.87%p 높아요`).
제목 색도 판정(세다/약하다)이 아니라 **오늘의 방향**을 따른다(`verdict.tone`) — 덜 오른 날을 파랑으로 칠하면 다시 하락으로 읽힌다.
이 줄에 오늘 절대값이 들어가 LIVE 바와 약 2분 시차로 숫자가 다를 수 있지만, 머리의 "어제 09:28 vs 오늘 09:28"이 기준 시각을 밝힌다 —
아래 근거 줄 "가격"은 여전히 오늘 절대값을 다시 적지 않는다.

**마감 후 데이터 규칙(§48·§51 재확인)**: 종목(주도주 타일·섹터 대표)의 기준가는 직전 거래일
**15:30 1분봉**이다 — 애프터장이 섞이는 종목에서만 이 규칙이 필요하다. **지수(코스피)의 기준가는
일봉 `day` 엔드포인트 종가다** — 지수는 애프터장 세션 자체가 없어 일봉 종가가 그대로 공식값이고,
`api/_vs-prices.mjs`의 `prevClose(kind='index', ...)`는 처음부터 그렇게 짜여 있었다. 2026-09-16
마감 브리핑 발행값(6,717.97 ▲90.71)으로 역산한 9/15 기준가 6,627.26이 일봉 종가와 정확히 일치했고,
같은 시각 1분봉(6,627.24)과는 소수점에서 갈렸다 — 이 문서에 처음 적었던 "기준가는 1분봉으로
통일한다"는 이 검증 전의 잘못된 판단이었다. 마감 후 수급은 **15:31~15:40 행** — 이 행으로 뽑은
9/15 수급(외국인 −15,711억 · 기관 −9,040억 · 개인 +8,320억)이 그날 마감 브리핑 발행값과 정확히
일치해 교차 검증됐다. 16:00 이후 실시간·일봉은 애프터장이 섞여 쓰지 않는다(§48). `status:'early'`는
15:30~15:39(수급 집계 공백)에 뜬다.

**2026-09-16 실측으로 확정한 한계**: 1분봉 조회는 약 7거래일까지(9/16 기준 9/8까지 조회되고 9/4는
0건). 지수 1분봉엔 거래대금이 없고 거래량(천주)만 있어 라벨을 **"거래량"**으로 쓴다("거래대금"이라
쓰면 §0 위반). 원/달러 환율은 분봉 자체가 없어 이번 범위에서 제외. 시간대별 수급 표는 과거 날짜도
페이지네이션으로 조회된다(1페이지만 보면 빈손으로 보인다 — 조사 중 실제로 한 번 잘못 판단했다).

**옛 "어제와 달라진 것" 카드는 뺐다(2026-09-17).** 개인·외국인·기관 누적 순매수를 가로 막대로 그리던 §49 카드가 새 "누가 사는가" 카드와 같은 숫자를 한 번 더 보여줬다. 그 카드 안에 있던 이슈 비교 렌더(C2로 꺼져 있던 것)도 함께 빠졌다 — 서버 계산 코드(`_vs-issues.mjs`)와 사전은 그대로라, 이슈 비교를 다시 켤 땐 화면을 새로 그려야 한다.

**섹터는 "대표 3종목 평균"이지 섹터 평균이 아니다.** 46종목 평균과 순위가 갈린다 — 9/16 13:30
실측에서 반도체가 46종목 기준 1위인데 3종목 기준으론 전력기기가 1위, 2차전지는 7위 vs 4위로
갈렸다. 조회량을 92건→48건으로 줄이려 3종목을 택한 트레이드오프이므로 표 제목에 "대표 3종목"을
박고 종목 이름을 함께 보인다. 일부 결측이면 행에 실제 종목 수(n)를 적는다 — "3종목 대표"로
뭉개지 않는다.

**실제로 겪은 결함과 방지 룰**:
- `amp.diff`가 반올림된 두 값끼리를 뺐다(계획 문서의 예시 코드 자체에 그렇게 쓰여 있었다) →
  미반올림 값으로 계산하도록 고쳤다. **설계 문서가 계획 문서의 예시 코드보다 우선한다** — 계획은
  기록이라 수정하지 않고, 구현만 설계를 따른다.
- `minuteBars()`가 `{t,v}`만 남기고 고가·저가·거래량을 버려, 실API로 조회하니 강도 축이 통째로
  비었고(`heatAxis` → vol `{t:0,y:0}`, amp `{t:null,y:null}`) 거래량 결측이 `null`이 아니라 `0`이라
  "0천주"라는 지어낸 숫자가 나갈 뻔했다(§0). 앞선 세 태스크의 단위 테스트는 가짜 봉에 h/l/vol을
  직접 채워 넣어 이 결함을 못 잡았다. → **순수 계산 모듈 테스트만으로는 부족하다. 실API 응답
  형태 그대로의 픽스처로 수집→계산을 잇는 테스트를 반드시 둔다.** 없으면 결측은 `0`이 아니라
  `null`로 낸다(§0 — 값이 없으면 지어내지 않는다).
- 기관 세부 6칸 중 값이 `null`인 항목에 2px 상승색 막대가 그려졌다(다른 미니바와 달리 null 가드가
  없었다) → 값이 없으면 막대 자체를 그리지 않는다.
- `vs=close`가 밤새 60초 폴링을 그대로 받게 됐는데 CDN `s-maxage=60`이라, 이미 확정된 같은 값을
  만들려고 원천 조립(약 110건 네이버 조회)이 **분당 1회**씩 밤새 돌 뻔했다 → `status:'ok'`(확정값)
  응답만 `s-maxage=1800`, 그 밖(`early`·`closed`·`waiting`)은 60 유지(`closeCacheControl`). §49의
  "대결판은 하루 몇 번만 조립된다"는 전제가 카드가 늘면서 깨질 수 있다는 것을 여기서 처음 확인했다.

**조회량(2026-09-17 09:48 장중 실측)**: 같은 fetch 참조로 조립 3회 시 129 → 32 → 32건(첫 회는 어제
값·기준가를 적재, 이후는 오늘 봉만). 기존 대결판(§49)은 조립당 8건이었으니 축 추가로 4배가 됐다.
**메모(`memoFor`)는 fetch 함수 참조별이다** — 조회량을 측정할 때 매번 새 fetch 래퍼를 만들면 메모
적중이 전혀 안 된 것처럼 보인다(측정 코드 자체의 함정이니 재현할 땐 같은 참조를 재사용할 것).

**남은 갭**: 밤 00:00~07:30엔 `vs=close`가 그 시각 기준 새 날짜로 closed를 반환해 강도 카드가
사라진다(고치려면 서버가 "직전 세션"을 고르게 바꿔야 한다). 토요일 00:10은 `slotOf`가 weekend로
분류해 금요일 밤 카드가 자정에 사라진다. 주도주 3종목(005930·000660·005380)과 섹터 대표가 겹쳐
오늘 봉 3건이 조립마다 중복 조회된다(§18과 같은 "조회량 중복"이지만 6%대라 이번엔 정리하지 않기로
판단했다). 주말 주간 비교는 미구현.

- **방지 룰(설계 문서가 계획 예시 코드보다 우선한다)**: 계획 단계에서 짜둔 예시 코드는 설계가
  명시한 규칙과 어긋날 수 있다(반올림 순서 같은 세부는 특히). 구현이 계획과 다르게 나왔다면
  계획이 틀렸을 가능성부터 의심하고, 계획 문서 자체는 기록이므로 고치지 않는다.
- **방지 룰(가짜 봉으로 만든 단위 테스트는 원천 응답 모양의 변화를 못 잡는다)**: 파서·집계 함수를
  테스트할 때 입력을 손으로 만들면 "그 함수가 옳게 계산하는가"만 검증되고 "원천이 실제로 그 필드를
  주는가"는 검증되지 않는다. 새 필드를 소비하는 계산을 추가할 때는 실API 응답을 캡처한 픽스처로
  최소 1개의 수집→계산 통합 테스트를 둔다.
- **방지 룰(카드 수를 늘리면 캐시 정책을 다시 계산한다)**: 응답에 "확정값이라 다시 안 변하는 값"과
  "아직 집계 중이라 곧 바뀔 값"이 섞여 있으면 캐시 헤더를 한 값으로 통일할 수 없다. 폴링 주기 ×
  조립 비용을 새로 곱해 "밤새 분당 조립"처럼 감춰진 배수 증가가 없는지 확인한다.
- **재발 시 진단 순서**: ① 강도 축이 비면 응답의 `axes.heat`를 보고, `minuteBars` 원본 반환에
  `h`·`l`·`vol`이 있는지 확인한다(§6 결함의 재발 형태). ② 마감 후 수급이 이상하면
  `investorDealTrendTime.naver`의 15:31~15:40 행을 그날 마감 브리핑 발행값과 직접 대조한다(§51의
  교차 검증 방법 그대로). ③ 조회량이 튀면 `vs=close` 응답 헤더의 `s-maxage`와 그 시각이
  `slotOf`의 어느 폴링 슬롯에 해당하는지 먼저 본다.

### 55. 상승 모멘텀 종목 블록 제거 (2026-09-17 결정)

사용자 요청으로 `/stocks/` 홈의 **⚡ 상승 모멘텀 종목(`#mom-track`)을 통째로 뺐다.** 아침 코스피 브리핑 픽의 진입·목표·손절을 장중 가격으로 추적하던 블록이다.

- 지운 것: `web/stocks/index.html`의 `#mom-track` 블록, `stocks-home.js` 블록 3(픽 로드·90초 가격 폴링·상태 배지), `#mom-track` 해시 스크롤 처리, `ueGate`의 모멘텀 표시 분기, `stocks-home.css`의 `#mom-track`·`.mt-*`, 용어집 `#momentum` 항목, 코스피 브리핑 템플릿(`sections/stock_picks.html`)의 "종목 시그널에서 트래킹 →" 링크와 `.picks-track-link` 스타일.
- **전용 API는 없었다.** 픽은 커밋된 `analysis_snapshot.json`을 읽었고, 가격은 공용 `/api/stocks-live`를 불렀다. 그 라우트는 주도주 타일·섹터 박스·섹터/미국 상세 페이지·브리핑 사이드바가 계속 쓰므로 남긴다. 이번 제거로 줄어드는 건 **장중 90초마다 한 번씩 나가던 픽 3종목 가격 조회**다.
