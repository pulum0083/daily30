...
미국 브리핑(21:15 KST)이 내건 "오늘 점검할 이슈"의 **결과**를 다음날 코스피 아침
브리핑(07:25)에서 실측으로 채점해 보여준다. 기존 "간밤 미국 시장 이슈"(`us_issues`)
...
- **숫자·판정·문구가 전부 결정론이다. LLM이 개입하지 않는다** — §0 위반 여지가 구조적으로 없다.
  결과 한 줄("4종목 중 2종목이 내렸어요 · 평균 −0.36%")도 LLM 문장이 아니라 실측 조립 템플릿이다.
...
| 롱테일 라벨("원가 민감 소비재·운송" 등, 13%) | 채점하지 않음 → 카드째 생략 |

프록시 화이트리스트에 **섹터가 섞인 라벨("지수 전반, 특히 성장주")을 넣지 말 것.** S&P500 하나로
대표할 수 없다 — 매핑을 늘릴수록 실측이 아니라 추론이 늘어난다.

**중복 채점 합치기**: 같은 프록시·같은 방향이 다시 나오면 뒤엣것은 제목만 남긴다. 2026-09-03
