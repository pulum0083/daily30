...
**표시 규칙:**
- 등락은 %가 아니라 **포인트 차이**(`-3.7p`). 0~100 점수라 등락률은 의미가 없다.
  `prev_close`가 없으면 `0.0p`로 채우지 않고 비운다(보합이라는 틀린 주장 방지).
- 뱃지 색은 기존 `vix-badge` 팔레트 재사용. 공포·탐욕은 **양 끝이 모두 위험 구간**이라
  극단(<25, ≥75)=red(`high`), 그 안쪽=yellow(`elevated`), 중립=green(`calm`).
...
테스트: `scripts/test_market_items.py`(8) — 2개 항목만 나오는지, F&G 포맷·라벨 폴백·신선도
게이트·전일값 결측 처리.
