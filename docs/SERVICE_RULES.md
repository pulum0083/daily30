...
- **긴 설명 2문단은 `<details>`로 기본 접힘**(2026-09-02 사용자 요청). 한 줄 요약만 남기고
  "이 섹터를 어떻게 보나요"를 눌러야 펼쳐진다 — 그 전엔 설명이 섹터 평균·종목 카드를 첫
  화면 밖으로 밀어냈다. **접혀 있어도 크롤러는 `<details>` 안 내용을 읽으므로 본문 분량에는
  손해가 없다**(측정: 접기 전후 2,124자 → 2,138자). 하단 "이 페이지 읽는 법"은 펼친 상태 유지.
