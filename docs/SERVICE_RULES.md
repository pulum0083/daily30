...
- **곡선을 D안으로 바꿨다(2026-09-17).** 색 띠를 빼고 오늘 곡선을 매끄럽게 그려 아래를 0% 기준 옅은 면(위 빨강·아래 파랑)으로 채운다. 어제는 점선 한 줄. 두 선 사이 차이는 곡선 아래 칸에 **샘플 분마다 0축 세로 막대**(오늘−어제)로 그리고 끝에 지금 차이 %p를 적는다. 막대는 같은 분에 두 값이 모두 있는 점만 쓴다(§0). 곡선 안 −%p 글자는 남은 장 칸에만 두고(왼쪽으로 넘기면 곡선을 덮는다) 장 끝 무렵·모바일에선 빼며, 아래 칸 끝 값이 대신한다. 매끄러운 곡선은 샘플 점을 지나지만 점 사이에서 살짝 넘칠 수 있다 — 값은 마우스 오버가 실제 샘플 점만 보여준다. 시안 `docs/prototypes/2026-09-17-vs-chart-styles.html`.
