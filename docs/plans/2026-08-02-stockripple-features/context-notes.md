...
- **재개 지점 = Task 5.** 계획 파일에 각 태스크의 전체 코드·테스트·커밋 명령이 그대로 적혀 있어 그것만 보고 이어가면 된다. 실행 방식은 `superpowers:subagent-driven-development`(태스크마다 구현 서브에이전트 → 스펙 준수 리뷰 → 코드 품질 리뷰 → 다음 태스크).
