...
### 1. 서브탭 IA — **구현 완료 (Task 1~6, 6개 커밋), 로컬 커밋만·미푸시**

- 스펙: [`docs/superpowers/specs/2026-08-02-stocks-subnav-ia-design.md`](../../superpowers/specs/2026-08-02-stocks-subnav-ia-design.md) (커밋 `dab819c6`, ncai 규격 반영 `adb680a9`, 2026-08-04 구현 중 드러난 전제 오류 정정 — 배치 절 참조)
- 구현 계획: [`docs/superpowers/plans/2026-08-03-stocks-subnav-ia.md`](../../superpowers/plans/2026-08-03-stocks-subnav-ia.md). 실행 방식 `superpowers:subagent-driven-development`(태스크마다 구현 서브에이전트 → 스펙 준수 리뷰 → 코드 품질 리뷰, 지적 사항은 같은 에이전트에 재작업 지시 후 재검토).
- 확정 구조: GNB는 `브리핑 | 월배당` 유지. 시그널 서브탭 6개 — **전체 · 특이신호 · 테마 · 섹터 · ETF · 일정**. 이번 구현 범위는 **기존 4개(전체·특이신호·섹터·ETF)만 점등**, 테마·일정은 `ds-subnav.js`의 `TABS`에 주석 처리해 두고 각 기능이 완성될 때 그 작업에서 점등.
- 배치: 페이지 머리말 끝(검색창·시장패널 아래), 언더라인 탭, 공용 스크립트 1벌(`ds-subnav.js`)로 세 페이지(`/stocks/`·`/themes/`·`/calendar/`) 공유 — §30 이중 구현 재발 방지. **구현 중 스펙의 전제가 틀렸음이 드러남** — 히어로·검색창·시장패널이 실제로는 `#home`(`.screen`) 안에 있어 탭 전환 시 함께 사라지는 구조였다. Task 1에서 이 블록을 `.screen` 밖(페이지 레벨)으로 옮긴 뒤에야 스펙의 배치 문단이 성립했다.
- **목업 2라운드**: 1라운드(`docs/prototypes/2026-08-03-stocks-subnav-tabs.html`, `-additive.html`)는 홈 전면 재구성으로 잘못 짚어 사용자가 반려 — "지금 있는 걸 그대로 살리고 더보기만 탭으로" 방향으로 정정. 2라운드(`-fullsize.html`)에서 라이브 픽셀 폭(696/348, 684/360)까지 맞춘 풀사이즈로 재작업, 사용자 지시로 섹터 선택 칩(서브탭 바로 아래, 언더라인과 구분되는 먹색 채움 디자인)도 추가.
- **홈 실측 감사(2026-08-04)로 밝혀진 사실**: 라이브 홈이 15,095px인데 다른 화면으로 가는 링크가 "특이 신호 전체 보기" 단 하나였다. `#sector`·`#passive`·`#ranking`은 완성돼 있는데 홈에서 도달 불가. ETF는 홈에 대응 블록 자체가 없었다(본문에 "ETF" 단어가 한 번도 안 나옴). → Task 5(ETF 요약 블록 신설)·Task 6(섹터·패시브·순위 링크 복구)로 이어짐.
- **Task 5 코드 품질 리뷰가 §0 위반급 버그를 잡음**: `/api/signals`의 ETF 폴링이 전멸해도 `betting`이 `{downAmt:0,upAmt:0,downRatio:0,upRatio:100,levPct:null}`처럼 완전한 형태로 돌아와, 가드가 존재 여부만 보고 "+0.00%"를 실측처럼 렌더할 뻔했다. `downAmt===0 && upAmt===0`을 명시적으로 거부하도록 수정(§0 — 정합성 > 완전성).
- **이번에 고치지 않고 기록만 한 라이브 버그 4건** (계획 문서에 표로 남김, 사용자 지시로 서브탭 이후 별도 작업): ① `#sector` 섹터 전환 UI 부재(반도체 고정) ② 종목 랭킹 15행 `onclick`이 전부 `goStock('000660')` ③ 28종목 랭킹·패시브 쏠림 수치가 하드코딩 리터럴(§20류) ④ `stocks-snapshot.json`과 `/api/vol-top`이 같은 종목에 다른 값.
- **최종 상태**: JS 93+84 / Python 559 전체 통과. `web/data`·`web/briefings` 오염 없음. 로컬 6개 기능 커밋 + 스펙 정정 커밋, **푸시는 사용자 지시 대기**.
...
### 3. 밤사이 브리지 — 스펙·구현계획 완료, **구현 Task 5~8 미착수 (여기서 재개)**
...
- **자기검토로 실제 버그 하나 잡음**: 스냅샷 스테일 기준을 처음 2일로 썼다가, 금요일 16:33 스냅샷을 월요일 07:25에 읽으면 이미 ~2.6일 지나 매주 월요일 정상 데이터가 스테일로 오판될 뻔했다 → §20 선례대로 **5일**로 수정(본문/테스트 케이스가 4일·5일로 어긋나 있던 것도 `aa51fb1b`에서 통일). 새 신선도 게이트를 쓸 때마다 "가장 긴 정상 간격(주말·연휴)"을 먼저 계산해볼 것.
- **구현 계획 작성 완료**: [`docs/superpowers/plans/2026-08-02-kospi-comfort-bridge.md`](../../superpowers/plans/2026-08-02-kospi-comfort-bridge.md) — Part 1(위로카드, Task 1~4) + Part 2(브리지, **Task 5~8, 미착수**).
- 계획 파일에 각 태스크의 전체 코드·테스트·커밋 명령이 그대로 적혀 있다. 실행 방식은 `superpowers:subagent-driven-development`(태스크마다 구현 서브에이전트 → 스펙 준수 리뷰 → 코드 품질 리뷰 → 다음 태스크).

#### Task 5 상태 — 구현·커밋 완료(`a7fb0011`), **코드 품질 리뷰 지적 3건 미수정**

스펙 준수 리뷰는 ✅ 통과(반환 스키마·반올림·`>` 경계·라벨 출처 전부 실행으로 검증). 그런데 코드 품질 리뷰가 **실제 사고 이력에 근거한 Important 3건**을 찾았고 아직 안 고쳤다. **여기서부터 재개할 것.**

1. **[가장 중요] 스테일 게이트가 "최근인가"를 묻는데 정합성은 "직전 코스피 세션인가"를 요구한다** (`fetch_data.py:769`). 스냅샷 잡은 `continue-on-error: true`(§21)라 거래일에 스킵될 수 있고, **실제로 2026-07-14(화) 스냅샷이 없다**(§17·§18 사고일). 그날 이 기능이 살아 있었다면 07-15 07:25에 **07-13 스냅샷**(2일 전, 5일 창 안이라 통과)을 읽어 월요일 종가와 화요일 밤 미국장을 비교 — 한국 07-14 세션이 통째로 빠진 채 갭이 계산된다. **이게 정확히 이 기능이 막으려던 §24 이중계상이다.** 5일 창 자체는 연휴 대응으로 맞으니 좁히지 말고, `session_label.prev_kospi_session()`(로컬 캘린더, 네트워크 없음)로 **직전 세션 일치 여부**를 보도록 교체할 것. 리뷰어가 실제 캘린더로 검증함 — `prev_kospi_session(2026-07-20)` → `2026-07-16`(제헌절 스킵), 월요일→금요일 통과, 07-15 사고 케이스는 `None` 반환. 더불어 **반환값에 한국 세션 날짜를 실어** Task 8 템플릿이 "한국 7/31 마감"으로 라벨링할 수 있게 할 것(§24 방지룰 "직전 거래일을 실제 계산해 라벨을 만든다").
2. **US 라벨이 수집 안 된 티커까지 주장한다** (`fetch_data.py:795`·`801`). `us_label`은 `BRIDGE_US_LEG`의 고정 문자열인데 `us_vals`는 필터링된다 — `VRT` 결측 시 GEV 단독 수치에 "GE Vernova·Vertiv" 라벨이 붙는다(실측 확인). 멀티티커 3개 섹터(power·auto·finance) 해당. 바로 두 줄 아래 KR 레그는 이미 실제 기여 종목명만 모아 쓰고 있어 비대칭 — §20 "데이터와 안 맞는 라벨이 계속 렌더된다" 그대로다.
3. **테스트가 뒤 태스크가 기댈 선을 안 잡아준다** (`test_overnight_bridge.py`). ① `test_normal_case_returns_seven_sectors_no_ship`이 실제로는 **2행만** 만든다(픽스처에 KR 종목 4개뿐) — `len(rows) <= 7`은 사실상 무의미하고 이름이 오해를 부른다. 픽스처를 유니버스에서 파생시켜 `== 7`로 바꿀 것. ② §24 리플레이의 `gap_pp < 3.0`은 −50도 통과하고 부호를 뒤집어도 통과 — 결정론적이니 `us_change == -4.25 and kr_change == -5.72 and gap_pp == -1.5`로 고정할 것. ③ 하드닝 가드 3개(비-dict 입력·config 로드 실패·`stocks`가 list) 전부 미테스트.

Minor(선택): `_bridge_change_pct` docstring의 근거를 실제 이유(두 레그의 추출 규칙 통일·이중 조회 제거)로 바꿀 것 — 문자열 `change_pct` 경로는 실제로 도달 불가하다. `fetch_overnight_bridge` docstring의 "순수함수"도 과장(설정 파일 읽고 `datetime.now()` 호출).

### 4. 코스피 브리핑 위로 카드 재구성 — **구현 완료(Task 1~4), 로컬 커밋만·미푸시** (StockRipple 스코프 밖 애드혹 지시)
...
- **구현 완료 커밋 (Task 1~4)**: `0a0f2499`(명언 저장) → `80188684`(gitignore) → `861b66b9`(예외처리 확장) → `e128c9bf`(텔레그램 동기화) → `91b005c1`(list 가드) → `33e93ca1`(HTML 컨텍스트 주입) → `c73c4921`(템플릿·CSS C2) → `28a7c3bb`(구버전 마크업 하위호환).
- **리뷰에서 잡은 실제 버그 3건** (전부 수정 완료) — ① `_save_todays_quote`의 try/except가 파싱만 감싸 malformed 항목이 07:25 브리핑 발행 전체를 중단시킬 수 있었음 ② `pick_quote`가 `guru_quotes.json`이 list가 아니면 `random.choice`에서 `KeyError`로 텔레그램 발송 전체를 죽일 수 있었음 ③ **`.comfort-line`에서 `display:flex`를 `__row`로 옮긴 게 이미 발행된 12개 페이지에 회귀** — `style.css`가 전 페이지 공용이라 구버전 마크업에서 아이콘이 텍스트 위로 쌓임. `:not(:has(.comfort-line__row))`로 하위호환 규칙 추가. **교훈: 공용 CSS의 클래스 구조를 바꿀 때는 이미 발행된 정적 HTML이 그 구조를 그대로 갖고 있다는 걸 항상 확인할 것.**
- 남은 것: 없음(Part 1 완료). 다만 `reasons.html` 템플릿과 `.rz-*` CSS가 고아로 남았다 — 별도 정리 태스크로 큐잉됨(백그라운드 chip).
